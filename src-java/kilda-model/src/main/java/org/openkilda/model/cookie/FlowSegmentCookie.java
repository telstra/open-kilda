...
public class FlowSegmentCookie extends Cookie {
    ...
    static final BitField HA_SUB_FLOW_SERVER_42_FLAG  = new BitField(0x0001_0000_0000_0000L);
    ...
    static final BitField[] ALL_FIELDS = ArrayUtils.addAll(
            ...
            FLOW_LOOP_FLAG, FLOW_MIRROR_FLAG, Y_FLOW_FLAG, HA_SUB_FLOW_SERVER_42_FLAG, STATS_VLAN_ID_FIELD,
            SUB_TYPE_FIELD);
    ...
    public FlowSegmentCookie(FlowPathDirection direction, long flowEffectiveId) {
        this(CookieType.SERVICE_OR_FLOW_SEGMENT, direction, flowEffectiveId, false, false, false, false, 0, null);
    }
    ...
    private FlowSegmentCookie(CookieType type, FlowPathDirection direction, long flowEffectiveId, boolean looped,
                              boolean mirror, boolean yFlow, boolean haSubFlowServer42, int statsVlan,
                              FlowSubType subType) {
        super(makeValue(type, direction, flowEffectiveId, looped, mirror, yFlow, haSubFlowServer42, statsVlan, subType),
                type);
    }
    ...
    public FlowSegmentCookieBuilder toBuilder() {
        return new FlowSegmentCookieBuilder()
                ...
                .mirror(isMirror())
                .yFlow(isYFlow())
                .haSubFlowServer42(isHaSubFlowServer42())
    ...
    public boolean isHaSubFlowServer42() {
        return getField(HA_SUB_FLOW_SERVER_42_FLAG) == 1;
    }
    ...
    private static long makeValue(CookieType type, FlowPathDirection direction, long flowEffectiveId,
                                  boolean looped, boolean mirror, boolean yFlow, boolean haSubFlowServer42,
                                  int statsVlan, FlowSubType subType) {
        if (!VALID_TYPES.contains(type)) {
        ...
        if (haSubFlowServer42) {
            result = setField(result, HA_SUB_FLOW_SERVER_42_FLAG, 1);
        }
        if (subType != null) {
