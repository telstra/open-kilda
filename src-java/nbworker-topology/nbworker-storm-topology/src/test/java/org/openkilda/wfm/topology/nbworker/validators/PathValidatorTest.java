...
public class PathValidatorTest extends InMemoryGraphBasedTest {
    ...
    public void createTestTopology() {
        if (!isSetupDone) {
            ...
            createOneWayIsl(switchA, 6, switchTransit, 6, 10, 2_000_000, 10_000);
            createOneWayIsl(switchTransit, 6, switchA, 6, 10, 2_000_000, 10_000);

            createOneWayIsl(switchB, 7, switchTransit, 7, 15, 3_000_000, 20_000);
            createOneWayIsl(switchTransit, 7, switchB, 7, 15, 3_000_000, 20_000);
    ...
    public void whenNoSwitchOnPath_validatePathReturnsErrorResponseTest() {
        ...
        Set<String> actualErrorMessages = new HashSet<>(responses.get(0).getErrors());
        Set<String> expectedErrorMessages = new HashSet<>(Arrays.asList(
    ...
    public void whenValidPathButTooLowLatencyTier2_andLatencyStrategy_validatePathReturnsErrorResponseTest() {
        ...
        assertFalse(responses.get(0).getErrors().isEmpty());
        Set<String> expectedErrorMessages = new HashSet<>();
        expectedErrorMessages.add("Requested latency tier 2 is too low between end points: switch 00:00:00:00:00:00:00:"
                + "03 port 7 and switch 00:00:00:00:00:00:00:02 port 7. Requested 1 ms, but the link supports 3 ms");
        expectedErrorMessages.add("Requested latency tier 2 is too low between end points: switch 00:00:00:00:00:00:00:"
                + "01 port 6 and switch 00:00:00:00:00:00:00:03 port 6. Requested 1 ms, but the link supports 2 ms");
    ...
    public void whenMultipleProblemsOnPath_validatePathReturnsAllErrorResponseTest() {
        ...
        nodes.add(new PathNodePayload(SWITCH_ID_1, null, 6));
        nodes.add(new PathNodePayload(SWITCH_ID_3, 6, 7));
        nodes.add(new PathNodePayload(new SwitchId("00"), 7, null));
        PathValidateRequest request = new PathValidateRequest(PathValidationDto.builder()
                .nodes(nodes)
                .bandwidth(1000000L)
                .latencyMs(1L)
        ...
        assertEquals("There must be 5 errors in total: 2 not enough bandwidth (forward and reverse paths), "
                        + "1 link is not present, 1 switch is not found, and 1 latency",
                5, responses.get(0).getErrors().size());
        Set<String> expectedErrorMessages = new HashSet<>();
        expectedErrorMessages.add("The following switch has not been found: 00:00:00:00:00:00:00:00");
        ...
        expectedErrorMessages.add("There is no ISL between end points: switch 00:00:00:00:00:00:00:03 port 7 and"
                + " switch 00:00:00:00:00:00:00:00 port 7");
        assertEquals(expectedErrorMessages, new HashSet<>(responses.get(0).getErrors()));
    }
