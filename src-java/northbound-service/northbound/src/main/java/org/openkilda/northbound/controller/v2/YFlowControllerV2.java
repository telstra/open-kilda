...
public class YFlowControllerV2 extends BaseController {
    ...
    @ResponseStatus(HttpStatus.OK)
    public CompletableFuture<YFlow> getYFlow(@PathVariable(name = "y_flow_id") String flowId) {
        return flowService.getYFlow(flowId);
    }

    @ApiOperation(value = "Gets Y-flow paths", response = YFlowPaths.class)
    @GetMapping(value = "/{y_flow_id:.+}/paths")
    @ResponseStatus(HttpStatus.OK)
    public CompletableFuture<YFlowPaths> getYFlowPaths(@PathVariable(name = "y_flow_id") String flowId) {
        return flowService.getYFlowPaths(flowId);
    }

    @ApiOperation(value = "Updates Y-flow", response = YFlow.class)
    @PutMapping(value = "/{y_flow_id:.+}")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public CompletableFuture<YFlow> updateYFlow(@PathVariable(name = "y_flow_id") String flowId,
                                                @Valid @RequestBody YFlowUpdatePayload flow) {
        return flowService.updateYFlow(flowId, flow);
    }

    @ApiOperation(value = "Updates Y-flow partially", response = YFlow.class)
    @PatchMapping(value = "/{y_flow_id:.+}")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public CompletableFuture<YFlow> patchYFlow(@PathVariable(name = "y_flow_id") String flowId,
                                               @Valid @RequestBody YFlowPatchPayload flowPatch) {
        return flowService.patchYFlow(flowId, flowPatch);
    }

    @ApiOperation(value = "Deletes Y-flow", response = YFlow.class)
    @DeleteMapping(value = "/{y_flow_id:.+}")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public CompletableFuture<YFlow> deleteYFlow(@PathVariable(name = "y_flow_id") String flowId) {
        return flowService.deleteYFlow(flowId);
    }

    @ApiOperation(value = "Gets subordinate flows of Y-flow", response = SubFlowsDump.class)
    @GetMapping(value = "/{y_flow_id:.+}/flows")
    @ResponseStatus(HttpStatus.OK)
    public CompletableFuture<SubFlowsDump> getSubFlows(@PathVariable(name = "y_flow_id") String flowId) {
        return flowService.getSubFlows(flowId);
    }

    @ApiOperation(value = "Reroute Y-flow", response = YFlowRerouteResult.class)
    @PostMapping(path = "/{y_flow_id:.+}/reroute")
    @ResponseStatus(HttpStatus.ACCEPTED)
    public CompletableFuture<YFlowRerouteResult> rerouteYFlow(@PathVariable(name = "y_flow_id") String flowId) {
        return flowService.rerouteYFlow(flowId);
    }
}
