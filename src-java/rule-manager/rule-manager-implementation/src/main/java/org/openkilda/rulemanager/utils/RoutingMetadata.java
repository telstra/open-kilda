...
public class RoutingMetadata {
    ...
    static final BitField[] ALL_FIELDS = new BitField[] {
            ...
            HA_SUB_FLOW_TYPE_FIELD, OUTER_VLAN_FIELD, INPUT_PORT_FIELD};
    ...
    protected RoutingMetadata(
        ...
        this.value = setField(makeValue(lldpFlag, arpFlag, oneSwitchFlowFlag, outerVlanId, inputPort, haSubFlowType),
                MetadataType.ROUTING.getValue(),
                TYPE_FIELD);
        this.mask = setField(makeMask(lldpFlag, arpFlag, oneSwitchFlowFlag, outerVlanId, inputPort, haSubFlowType),
    ...
    public enum HaSubFlowType implements NumericEnumField {
        HA_SUB_FLOW_1(0),
        HA_SUB_FLOW_2(1);

        private final int value;

        HaSubFlowType(int value) {
            this.value = value;
        }
    ...
    private static long makeValue(
        ...
        if (haSubFlowType != null) {
            result = setField(result, haSubFlowType.getValue(), HA_SUB_FLOW_TYPE_FIELD);
        }
        return result;
    }

    private static long makeMask(Boolean lldpFlag, Boolean arpFlag, Boolean oneSwitchFlowFlag,
                                 Integer outerVlanId, Integer inputPort, HaSubFlowType haSubFlowType) {
        long result = 0;
        if (lldpFlag != null) {
        ...
        if (haSubFlowType != null) {
            result = setField(result, FULL_MASK, HA_SUB_FLOW_TYPE_FIELD);
        }
        return result;
    }
    ...
    public static class RoutingMetadataBuilder {
        ...
        private RoutingMetadata buildTruncatedTo32Bits() {
            ...
            return new RoutingMetadata(lldpFlag, arpFlag, oneSwitchFlowFlag, outerVlanId, inputPort, haSubFlowType);
        }
