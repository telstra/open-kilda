 ...
 */
class FermaLinkPropsRepository extends FermaGenericRepository<LinkProps, LinkPropsData, LinkPropsFrame>
        implements LinkPropsRepository {
    FermaLinkPropsRepository(FramedGraphFactory<?> graphFactory, TransactionManager transactionManager) {
    ...
    @Override
    protected LinkPropsData doRemove(LinkProps entity, LinkPropsFrame frame) {
        LinkPropsData data = LinkProps.LinkPropsCloner.INSTANCE.copy(frame);
        frame.remove();
        return data;
    }
}
