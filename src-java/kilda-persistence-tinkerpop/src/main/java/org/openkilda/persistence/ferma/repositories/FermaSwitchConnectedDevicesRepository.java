 ...
 */
class FermaSwitchConnectedDevicesRepository
    ...
    public Collection<SwitchConnectedDevice> findBySwitchId(SwitchId switchId) {
        return framedGraph().traverse(g -> FermaSwitchRepository.getTraverseForSwitch(g, switchId)
                .out(SwitchConnectedDeviceFrame.HAS_BY_EDGE)
                .hasLabel(SwitchConnectedDeviceFrame.FRAME_LABEL))
                .toListExplicit(SwitchConnectedDeviceFrame.class).stream()
                .map(SwitchConnectedDevice::new)
                .collect(Collectors.toList());
    ...
    @Override
    protected SwitchConnectedDeviceData doRemove(SwitchConnectedDevice entity, SwitchConnectedDeviceFrame frame) {
        SwitchConnectedDeviceData data = SwitchConnectedDevice.SwitchConnectedDeviceCloner.INSTANCE.copy(frame);
