...
public abstract class YSubFlowFrame extends KildaBaseEdgeFrame implements YSubFlowData {
    ...
    public YFlow getYFlow() {
        if (yFlow == null) {
            ...
            if (!Objects.equals(getYFlowId(), yFlowId)) {
                throw new IllegalStateException(format("The y_subflow %s has inconsistent yflow_id %s / %s",
    ...
    public Flow getSubFlow() {
        if (subFlow == null) {
            ...
            subFlow = !flowFrames.isEmpty() ? new Flow((flowFrames.get(0))) : null;
            String flowId = subFlow != null ? subFlow.getFlowId() : null;
            if (!Objects.equals(getSubFlowId(), flowId)) {
                throw new IllegalStateException(format("The y_subflow %s has inconsistent subflow_id %s / %s",
                        getId(), getSubFlowId(), flowId));
            }
        }
        return subFlow;
    }
