...
class HaFlowStats {
    ...
    @Autowired
    HaFlowStats(StatsHelper statsHelper){
        HaFlowStats.statsHelper = statsHelper
    }
    ...
    HaFlowStats(String haFlowId) {
        ...
        this.stats = statsHelper.getTsdb().queryDataPointsForLastMinutes(HaFlowStatsMetric.values() as List,
        "ha_flow_id", haFlowId, 60)
    }
    ...
    StatsResult get(HaFlowStatsMetric metric, Direction direction) {
        return this.stats.find {it.metric.equals(metricPrefix + metric.getValue()) && it.tags.get("direction").equals(direction.getValue())}
    }
