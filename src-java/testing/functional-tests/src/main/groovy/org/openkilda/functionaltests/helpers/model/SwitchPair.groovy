...
class SwitchPair {
    ...
    int getSizeOfTheShortestPath(){
        paths.min { it.size() }.size()
    }
    ...
    List<Path> retrieveAvailablePathsExcept(List<Path> pathToExclude) {
        def pathsList = convertToPathNodePayload(paths).collect{
            new Path(it, topologyDefinition)
        }
         pathsList.findAll { !pathToExclude.contains(it)}
    }
    ...
    List<Path> retrieveNextLongestPathsTo(Path path){
