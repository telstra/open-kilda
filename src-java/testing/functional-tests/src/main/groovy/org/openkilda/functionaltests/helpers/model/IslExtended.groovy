...
class IslExtended {
    ...
    void waitForRoundTripStatus(IslChangeType forwardExpectedStatus, IslChangeType reversedExpectedStatus = forwardExpectedStatus,
                                double timeout = KildaProperties.DISCOVERY_EXHAUSTED_INTERVAL + WAIT_OFFSET / 2) {
        wait(timeout) {
            List<IslInfoData> allLinksFromNb = northbound.getAllLinks()
    ...
    IslInfoData getInfo(List<IslInfoData> allLinksFromNb = northbound.getAllLinks(), boolean isReversed = false) {
        return allLinksFromNb.find { link ->
            def source = isReversed ? dstEndpoint : srcEndpoint
            def destination = isReversed ? srcEndpoint : dstEndpoint

            link.source.portNo == source.port && link.source.switchId == source.sw.dpId
                    && link.destination.portNo == destination.port
                    && link.destination.switchId == destination.sw.dpId
        }
    }

    boolean isIncludedInPath(List<IslExtended> path) {
        path*.isl.contains(isl) || path*.isl.contains(isl.reversed)
    }
    ...
    def delete(boolean isForce = false) {
        ...
        return northbound.deleteLink(toLinkParameters(), isForce)
    }

    def setBfd(BfdProperties props = new BfdProperties(350L, (short) 3)) {
    ...
    def setBfdFromApiV1(boolean isEnabled) {
        cleanupManager.addAction(OTHER, {deleteBfd()})
        return northbound.setLinkBfd(toLinkEnableBfd(isEnabled))
    }
    ...
    def setMaintenance(boolean isUnderMaintenance, boolean isEvacuate) {
        ...
        return northbound.setLinkMaintenance(toLinkUnderMaintenance(isUnderMaintenance, isEvacuate))
    }
    ...
    BatchResults updateCost(Integer cost) {
        ...
        northbound.updateLinkProps([toLinkProps(["cost": cost.toString()])])
    }
    ...
    def deleteCostProp(Integer cost){
        northbound.deleteLinkProps([toLinkProps(["cost": cost.toString()])])
    }
    ...
    def replugDestination(IslExtended plugIntoIsl, boolean plugIntoSource, boolean portDown,
        ...
        waitForStatus(expectedRepluggedIslState, KildaProperties.DISCOVERY_EXHAUSTED_INTERVAL + WAIT_OFFSET)
    ...
    LinkUnderMaintenanceDto toLinkUnderMaintenance(boolean underMaintenance, boolean evacuate) {
        return new LinkUnderMaintenanceDto(isl.srcSwitch.dpId.toString(), isl.srcPort,
                isl.dstSwitch.dpId.toString(), isl.dstPort, underMaintenance, evacuate)
    }

    LinkEnableBfdDto toLinkEnableBfd(boolean bfd) {
        return new LinkEnableBfdDto(isl.srcSwitch.dpId.toString(), isl.srcPort,
    ...
    LinkParametersDto toLinkParameters() {
        return new LinkParametersDto(isl.srcSwitch.dpId.toString(), isl.srcPort,
    ...
    LinkPropsDto toLinkProps(HashMap props) {
        return new LinkPropsDto(isl.srcSwitch.dpId.toString(), isl.srcPort,
