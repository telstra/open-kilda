    ...
    IslInfoData getInfo(List<IslInfoData> allLinksFromNb = northbound.getAllLinks(), boolean isReversed = false) {
        return allLinksFromNb.find { link ->
            ...
            source.switchId == srcSwId && source.portNo == srcPort
                    && destination.switchId == dstSwId && destination.portNo == dstPort
        }
    }

    def verifyAvailableBandwidthIsCorrect(List<IslInfoData> linksInfoBefore, List<IslInfoData> linksInfoAfter, Long usedBandwidth) {
        assert getInfo(linksInfoBefore).availableBandwidth == getInfo(linksInfoAfter).availableBandwidth + usedBandwidth
        assert getInfo(linksInfoBefore, true).availableBandwidth == getInfo(linksInfoAfter, true).availableBandwidth + usedBandwidth
    }
