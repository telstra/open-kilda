...
class Server42FlowRttSpec extends HealthCheckSpecification {
    ...
    def "Flow rtt stats are available in forward and reverse directions for new flows"() {
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT, 1) {
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValues()
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValues()
        }
    ...
    def "Flow rtt stats are available only if both global and switch toggles are 'on' on both endpoints"() {
        ...
        flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD) == null

        and: "Expect no flow rtt stats for reversed flow"
        flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE) == null
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT, 1) {
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(checkpointTime)
            //https://github.com/telstra/open-kilda/issues/4678
            //assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE).hasNonZeroValuesAfter(checkpointTime)
        }
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT + WAIT_OFFSET, 1) {
            ...
            assert stats.get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(checkpointTime)
            //https://github.com/telstra/open-kilda/issues/4678
            //assert stats.get(FLOW_RTT, REVERSE).hasNonZeroValuesAfter(checkpointTime)
        }
        ...
        then: "Expect no flow rtt stats for forward flow"
        !flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(checkpointTime)

        and: "Expect no flow rtt stats for reversed flow"
        !flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE).hasNonZeroValuesAfter(checkpointTime)
    ...
    def "Flow rtt stats are available if both endpoints are connected to the same server42, same pop"() {
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT, 1) {
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValues()
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE).hasNonZeroValues()
        }
    ...
    def "Able to synchronize a flow (install missing server42 rules)"() {
        ...
        !flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(timeWhenMissingRuleIsDetected)

        and: "server42 stats for reverse direction are increased"
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT + WAIT_OFFSET) {
            flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE).hasNonZeroValuesAfter(timeWhenMissingRuleIsDetected)
        }
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT + WAIT_OFFSET, 1) {
            flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(timeWhenMissingRuleIsReinstalled)
        }
    ...
    def "Able to swapEndpoint for a flow with enabled server42 on it"() {
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT, 1) {
            assert flowStats.of(flow1.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValues()
            assert flowStats.of(flow2.getFlowId()).get(FLOW_RTT, FORWARD) == null
        }
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT, 1) {
            assert flowStats.of(flow2.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValues()
        }

        and: "server42 stats are not available any more for the flow1 in the forward direction"
        !flowStats.of(flow1.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(timeWhenEndpointWereSwapped)
    ...
    def "Rtt statistic is available for a flow in case switch is not connected to server42"() {
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT, 1) {
            ...
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE, SERVER_42) == null
        }
        ...
        Wrappers.wait(flowSlaCheckIntervalSeconds * 3, 1) {
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD, FLOW_MONITORING) == null
    ...
    def "Flow rtt stats are still available after updating a #data.flowDescription flow"() {
        ...
        Wrappers.wait(STATS_FROM_SERVER42_LOGGING_TIMEOUT + WAIT_OFFSET, 1) {
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD).hasNonZeroValuesAfter(flowUpdateTime)
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE).hasNonZeroValuesAfter(flowUpdateTime)
        }
    ...
    def "Flow rtt stats are available after updating switch properties related to server42"(){
        ...
        Wrappers.timedLoop(STATS_FROM_SERVER42_LOGGING_TIMEOUT / 2) {
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, FORWARD) == null
            assert flowStats.of(flow.getFlowId()).get(FLOW_RTT, REVERSE) == null
        }
