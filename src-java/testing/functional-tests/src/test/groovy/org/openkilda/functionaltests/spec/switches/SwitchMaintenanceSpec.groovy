...
class SwitchMaintenanceSpec extends HealthCheckSpecification {
    ...
    def "Maintenance mode can be set/unset for a particular switch"() {
        ...
        swRelatedIsls.each {
            assert it.getCostFromDb() == DEFAULT_COST
            assert it.reversed.getCostFromDb() == DEFAULT_COST
        }
    ...
    def "Flows can be evacuated (rerouted) from a particular switch(several Isls are in use) when setting maintenance mode for it"() {
        ...
        def switchUnderTest = switches.all().getListOfSwitches().find{ it.islPorts.size() >= 3 }
        List<IslExtended> intermediateSwIsls = isls.all().relatedTo(switchUnderTest).getListOfIsls()

        def swPair = switchPairs.all().nonNeighbouring().excludeSwitches([switchUnderTest]).getSwitchPairs().find {
            it.paths.findAll { switchUnderTest.switchId in it.switchId }.size() >= 2
        ...
        List<List<IslExtended>> availablePaths =  swPair.retrieveAvailablePaths().collect { isls.all().findInPath(it) }
        List<IslExtended> flow1PathIsls = availablePaths.find{ path ->
            intermediateSwIsls.first().isIncludedInPath(path) && !intermediateSwIsls.last().isIncludedInPath(path)
        }
        availablePaths.findAll { it != flow1PathIsls }.each { isls.all().makePathIslsMorePreferable(flow1PathIsls, it) }

        and: "Create a Flow going through these switches"
        def flow1 = flowFactory.getRandom(swPair)
        def actualFlow1Path = flow1.retrieveAllEntityPaths()
        def actualFlow1Isls = isls.all().findInPath(actualFlow1Path)
        assert actualFlow1Isls == flow1PathIsls
        ...
        def additionalSwitchPair = switchPairs.all().nonNeighbouring()
                .excludeSwitches([swPair.src, switchUnderTest]).random()

        List<List<IslExtended>> additionalAvailablePaths = additionalSwitchPair.retrieveAvailablePaths().collect { isls.all().findInPath(it) }
        def flow2PathIsls = additionalAvailablePaths.find { path ->
            intermediateSwIsls.last().isIncludedInPath(path) && !intermediateSwIsls.first().isIncludedInPath(path)
        }

        additionalAvailablePaths.findAll { it != flow2PathIsls }.each { isls.all().makePathIslsMorePreferable(flow2PathIsls, it) }

        def flow2 = flowFactory.getRandom(additionalSwitchPair, false)
        def actualFlow2Path = flow2.retrieveAllEntityPaths()
        def actualFlow2Isls = isls.all().findInPath(actualFlow2Path)
        assert actualFlow2Isls == flow2PathIsls
        ...
        timedLoop(3) {
            assert flow1.retrieveAllEntityPaths() == actualFlow1Path
            assert flow2.retrieveAllEntityPaths() == actualFlow2Path
        }
        ...
        !flow1.retrieveAllEntityPaths().getInvolvedSwitches().contains(switchUnderTest.switchId)
        !flow2.retrieveAllEntityPaths().getInvolvedSwitches().contains(switchUnderTest.switchId)
    ...
    def "Both Y-Flow and Flow can be evacuated (rerouted) from a particular switch when setting maintenance mode for it"() {
        ...
        def swTriplet = profile == "hardware" ? switchTriplets.all().withSharedEpEp1Ep2InChain().random()
                : switchTriplets.all().nonNeighbouring().random()
        ...
        if (profile == "hardware") {
            ...
            def availablePaths = swTriplet.pathsEp1[0].size() == 2 ?
                    swTriplet.retrieveAvailablePathsEp1().collect { isls.all().findInPath(it) } :
                    swTriplet.retrieveAvailablePathsEp2().collect { isls.all().findInPath(it) }
            // 1 isl is equal to 2 pathNodes
            def preferablePath = availablePaths.find { it.size() > 1 }
            availablePaths.findAll { it != preferablePath }.each { isls.all().makePathIslsMorePreferable(preferablePath, it) }
        }
        ...
        def pathIsls = availablePaths.find { path -> path.involvedSwIds.flatten().contains(transitSw.switchId) }
    ...
    def "Link discovered by a switch under maintenance is marked as maintained"() {
        ...
        def isl = isls.all().relatedTo(switchUnderTest).first()
        ...
        Wrappers.wait(discoveryInterval + WAIT_OFFSET) {
            ...
            def islInfo = isl.getInfo(links, false)
            def reverseIslInfo = isl.getInfo(links, true)
