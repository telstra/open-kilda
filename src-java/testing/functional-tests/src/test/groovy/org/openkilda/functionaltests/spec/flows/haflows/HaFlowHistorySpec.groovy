...
class HaFlowHistorySpec extends HealthCheckSpecification {
    ...
    def setupSpec() {
        def swT = topologyHelper.getSwitchTriplets().first()
        def haFlowRequest = haFlowHelper.randomHaFlow(swT)
        presetHaFlow = haFlowHelper.addHaFlow(haFlowRequest)

    }


    @Tidy
    def "HaFl0w has create event in its history"() {
        when: "Create a flow"

        Long timestampAfterCreate = System.currentTimeSeconds()
        def haflowHistory = northboundV2.getHaFlow(flow.flowId, specStartTime, timestampAfterCreate)
...
//            assert flowInfo.statusInfo == "ValidateFlowAction failed: Flow's $flow.flowId src switch is not active"
    ...
    def cleanupSpec() {
     haFlowHelper.deleteHaFlow(presetHaFlow)
    }
