...
class AutoRerouteSpec extends HealthCheckSpecification {
    ...
    def "Flow in 'Down' status is rerouted when discovering a new ISL"() {
        ...
        def broughtDownIsls = isls.all().relatedTo(switchPair.src).getListOfIsls()
        breakIsls(broughtDownIsls)
        ...
        restoreIsls(broughtDownIsls.findAll{ !it.isIncludedInPath(flowInvolvedIsls) })
    ...
    def "Flow in 'Up' status is not rerouted when discovering a new ISL and more preferable path is available"() {
        given: "A flow with one alternative path at least"
        def switchPair = switchPairs.all().neighbouring().withAtLeastNPaths(2).random()
        def allFlowPaths = switchPair.retrieveAvailablePaths().collect { isls.all().findInPath(it) }
        ...
        allFlowPaths.findAll { it != initialFlowIsls }.each { makePathIslsMorePreferable(it, initialFlowIsls, northbound.getAllLinks()) }
    ...
    def "Flow in 'Up' status is not rerouted when connecting a new switch and more preferable path is available"() {
        given: "A flow with one alternative path at least"
        def switchPair = switchPairs.all().neighbouring().withAtLeastNPaths(2).random()
        def allFlowPathsIsls = switchPair.retrieveAvailablePaths().collect { isls.all().findInPath(it) }
        ...
        and: "Make the current flow path less preferable than others"
        allFlowPathsIsls.findAll { it != initialFlowIsls }.each { makePathIslsMorePreferable(it, initialFlowIsls, northbound.getAllLinks()) }
    ...
    def "Flow is not rerouted when one of the flow ports goes down"() {
        ...
        and: "Make the current flow path less preferable than others"
        allFlowPathsIsls.findAll { it != flowIsls }.each { makePathIslsMorePreferable(it, flowIsls, northbound.getAllLinks()) }
    ...
    def "System properly handles multiple flow reroutes if ISL on new path breaks while first reroute is in progress"() {
        ...
        availablePathsIsls.findAll { it != mainIsls }.each { makePathIslsMorePreferable(mainIsls, it, northbound.getAllLinks()) }
        ...
        isls.all().deleteAllIslsProps()

        availablePathsIsls.findAll { it != backupIsls }.each { makePathIslsMorePreferable(backupIsls, it, northbound.getAllLinks()) }
...
class AutoRerouteIsolatedSpec extends HealthCheckSpecification {
    ...
    def "Flow in 'Down' status is rerouted after switchUp event"() {
        ...
        def shortestPaths = switchPair1.retrievePathsWithNodesCount(switchPair1.getSizeOfTheShortestPath())
        ...
        def altPaths1 = switchPair1.retrieveAvailablePathsExcept([firstFlowMainPath, firstFlowBackupPath])
        def altPaths2 = switchPair2.retrieveAvailablePathsExcept([secondFlowPath])

        def islsToBreak = isls.all()
                .collectIslsFromPaths((altPaths1 + altPaths2).flatten() as List<Path>)
                .excludeIsls(untouchableIsls).unique()

        breakIsls(islsToBreak)

        //firstFlowMainPath path more preferable than the firstFlowBackupPath
        makePathIslsMorePreferable(firstFlowMainIsls, firstFlowBackupIsls, northbound.getAllLinks())
