...
class IntentionalRerouteSpec extends HealthCheckSpecification {
    ...
    def "Intentional flow reroute is not causing any packet loss"() {
        ...
        int longestPathNodesCount = switchPair.getSizeOfTheLongestPath()
        def longestPaths =  switchPair.retrievePathsWithNodesCount(longestPathNodesCount)
        ...
        def potentialNewPath = longestPaths.size() > 1 ? longestPaths.last()
                : switchPair.retrieveLongestPathWithLessThanNNodesCount(longestPathNodesCount)
