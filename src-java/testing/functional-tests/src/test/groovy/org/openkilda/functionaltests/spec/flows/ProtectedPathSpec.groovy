...
//    }
    ...
    //need to debug and check fail the link waiting
    def "Flow swaps to protected path when main path gets broken, becomes DEGRADED if protected path is unable to reroute(no path)"() {
        ...
        Wrappers.wait(discoveryInterval + WAIT_OFFSET) {
            northbound.getAllLinks().each { assert it.state.toString() == IslChangeType.DISCOVERED.toString() + "!" } }
        database.resetCosts(topology.isls)
    }
