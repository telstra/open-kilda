...
class ProtectedPathSpec extends HealthCheckSpecification {
 ...
 is intentional"() {
        ...
        alternativePaths.each { makePathIslsMorePreferable(it, initialMainPathIsls, northbound.getAllLinks())}
        alternativePaths.each { makePathIslsMorePreferable(it, initialProtectedPathIsls, northbound.getAllLinks()) }
 ...
 is automatical"() {
    ...
    def "Able to create a flow with protected path when there is not enough bandwidth and ignoreBandwidth=true"() {
        ...
        isls.all().relatedTo(swPair.src).excludeIsls([directIsl])
                .updateIslsAvailableBandwidth(bandwidth - 1)
...
(altPath is more preferable than mainPath)"() {
        ...
        def broughtDownIsls = isls.all().relatedTo(switchPair)
                .excludeIsls(mainPathIsl + protectedIslToBreak).getListOfIsls()
        breakIsls(broughtDownIsls)
        ...
        availablePathsIsls.findAll { !it.containsAll(alternativeIsl) }
                .each { makePathIslsMorePreferable(alternativeIsl, it, northbound.getAllLinks()) }
