...
class Server42HaFlowRttSpec extends HealthCheckSpecification {
    ...
    def "Create an Ha-Flow (#description) with server42 Rtt feature and check datapoints in tsdb"() {
        ...
        and: "Create Ha-Flow"
        HaFlowExtended haFlow = haFlowRequest(HaFlowExtended.build(swT, northboundV2, topology)).create()
        ...
        where:
        description                                                                  | haFlowRequest
        "default flow encapsulation TRANSIT_VLAN"                                    | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withSharedEndpointFullPort().withEp1FullPort().withEp2FullPort() }
        "default flow encapsulation VXLAN"                                           | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withSharedEndpointFullPort().withEp1FullPort().withEp2FullPort().withEncapsulationType(VXLAN) }
        "shared ep is the full port and encapsulation VXLAN"                         | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withSharedEndpointFullPort().withEncapsulationType(VXLAN) }
        "shared ep qnq and encapsulation TRANSIT_VLAN"                               | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withSharedEndpointQnQ() }
        "tagged flow encapsulation VXLAN"                                            | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withEncapsulationType(VXLAN) }
        "ep1 and ep2 are on the same switch and port and encapsulation TRANSIT_VLAN" | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withEp1AndEp2SameSwitchAndPort() }
        "ep1 is the full port and encapsulation TRANSIT_VLAN"                        | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withEp1FullPort() }
        "all endpoints qnq  and encapsulation TRANSIT_VLAN"                          | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withEp1QnQ().withEp2QnQ().withSharedEndpointQnQ() }
        "protected path"                                                             | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withProtectedPath(true) }
        "ep1+ep2 qnq and encapsulation TRANSIT_VLAN"                                 | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withEp2QnQ().withEp1QnQ() }
//        known issue(qnq and VXLAN(ovs)) https://github.com/telstra/open-kilda/issues/4572
//        "ep2 qnq and encapsulation VXLAN"                                            | { HaFlowBuilder haFlowBuilder -> haFlowBuilder.withEp2QnQ().withEncapsulationType(VXLAN) }
