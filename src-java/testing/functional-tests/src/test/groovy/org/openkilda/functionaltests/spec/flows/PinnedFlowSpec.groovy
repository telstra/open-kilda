...
class PinnedFlowSpec extends HealthCheckSpecification {
    ...
    def "System doesn't reroute(automatically) pinned flow when flow path is partially broken"() {
        ...
        allPaths.findAll { it != longestPathIsls }
        ...
        allPaths.findAll { it != altPathIsls }.each { makePathIslsMorePreferable(altPathIsls, it, northbound.getAllLinks()) }

        and: "Init reroute by bringing current path's ISL down one by one"
        def islsToBreak = longestPathIsls.findAll { !it.isIncludedInPath(altPathIsls) }
    ...
    def "System is not rerouting pinned flow when 'reroute link flows' is called"() {
        ...
        availablePaths.findAll { it != newPathIsls }.each { makePathIslsMorePreferable(newPathIsls, it, northbound.getAllLinks()) }
    ...
    def "System returns error if trying to intentionally reroute a pinned flow"() {
        ...
        availablePaths.findAll { it != newPath }.each { makePathIslsMorePreferable(newPath, it, northbound.getAllLinks()) }
