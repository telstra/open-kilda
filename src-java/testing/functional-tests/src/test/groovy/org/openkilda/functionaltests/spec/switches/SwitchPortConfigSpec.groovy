...
class SwitchPortConfigSpec extends HealthCheckSpecification {
    ...
    def "Able to bring ISL-free port down/up on #sw.hwSwString()"() {
        ...
        when: "Bring port down on the switch"
        def port = sw.getRandomPortInState("LINK_DOWN" )

        port.down()

        then: "Port is really DOWN"
        Wrappers.wait(WAIT_OFFSET) { assert "PORT_DOWN" in port.retrieveDetails().config }
        ...
        sw << switches.all().uniqueByHw().findAll { it.ofVersion != "OF_12" && !it.wb5164 }
