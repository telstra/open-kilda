...
'${NON_EXISTENT_SWITCH_ID}_${port}': Switch ${NON_EXISTENT_SWITCH_ID} not found.", ~/Persistence exception/).matches(exc)    }
    ...
    def "System doesn't discover link when port discovery property is disabled"() {
        ...
        def isRtl = switches.all().findSpecific(islToManipulate.involvedSwIds)
                .any { it.getDbFeatures().contains(SwitchFeature.NOVIFLOW_COPY_FIELD) }
        ...
        then: "Link is detected and status of REVERSE ISL is FAILED"
        Wrappers.wait(discoveryInterval + WAIT_OFFSET) {
            def islInfo = islToManipulate.reversed.getNbDetails()
            assert islInfo.actualState == FAILED
            assert islInfo.state == (isRtl ? DISCOVERED : FAILED)
        }

        and: "Link is detected and status of FORWARD ISL is DISCOVERED"
        verifyAll(islToManipulate.getNbDetails()) { islInfo ->
            islInfo.actualState == DISCOVERED
            islInfo.state == (isRtl ? DISCOVERED : FAILED)
        }
    ...
    def "Link is stopped from being discovered after disabling port discovery property"() {
        ...
        then: "REVERSED ISL status is changed to FAILED"
        Wrappers.wait(discoveryTimeout + WAIT_OFFSET) {
            def reversedIslInfo = islToManipulate.reversed.getNbDetails()
            reversedIslInfo.actualState == FAILED
            reversedIslInfo.state == (isRtl ? DISCOVERED : FAILED)
        }

        and: "FORWARD ISL status remains DISCOVERED"
        verifyAll(islToManipulate.getNbDetails()) {
            actualState == DISCOVERED
            state == (isRtl ? DISCOVERED : FAILED)
        }
