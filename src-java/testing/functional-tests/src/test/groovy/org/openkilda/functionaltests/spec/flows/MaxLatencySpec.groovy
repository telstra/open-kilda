...
but satisfies max_latency_tier2"
    ...
    def setLatencyForPaths(int mainPathLatency, int alternativePathLatency) {
        ...
        database.updateIslsLatency([mainIsls[0], mainIsls[0].reversed], mainIslCost + (mainPathLatency % mainIsls.size()) * nanoMultiplier)
        mainIsls.size() > 1 && database.updateIslsLatency(mainIsls.tail().collectMany { [it, it.reversed] }, mainIslCost)
        database.updateIslsLatency([alternativeIsls[0], alternativeIsls[0].reversed], alternativeIslCost + (alternativePathLatency % alternativeIsls.size()) * nanoMultiplier)
