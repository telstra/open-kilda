...
class MaxLatencySpec extends HealthCheckSpecification {
    ...
    def setupSpec() {
        ...
        islsToBreak = isls.all().relatedTo(switchPair).excludeIsls(mainIsls + alternativeIsls).getListOfIsls()
    ...
    def "Able to create protected flow with max_latency strategy if both paths satisfy SLA"() {
        ...
        isls.all().findInPath(initialPath.flowPath.path.forward) == alternativeIsls
        isls.all().findInPath(initialPath.flowPath.protectedPath.forward) == mainIsls
    }
    ...
    def "Able to create DEGRADED protected flow with max_latency strategy if maxLatency < protectedPathLatency < maxLatencyTier2"() {
        ...
        def initialPath = flow.retrieveAllEntityPaths()
        isls.all().findInPath(initialPath.flowPath.path.forward) == mainIsls
        isls.all().findInPath(initialPath.flowPath.protectedPath.forward) == alternativeIsls
    }
    ...
    def "Able to create DEGRADED flow with max_latency strategy if maxLatencyTier2 > pathLatency > maxLatency"() {
        ...
        isls.all().findInPath(flow.retrieveAllEntityPaths()) == alternativeIsls
    }
    ...
    def "Able to update DEGRADED flow with max_latency strategy if maxLatencyTier2 > pathLatency > maxLatency"() {
        ...
        assert isls.all().findInPath(flow.retrieveAllEntityPaths()) == alternativeIsls
    }
