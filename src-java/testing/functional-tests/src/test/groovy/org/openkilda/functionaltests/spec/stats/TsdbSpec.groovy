...
class TsdbSpec extends HealthCheckSpecification {
    ...
    def setupSpec() {
        withPool {
            statsMap = uniqueSwitches.collectParallel {[it, switchStats.of(it, 15)]}
    ...
    @Tags([HARDWARE])
    def "GRPC stats are being logged"(metric, switchId) {
        assumeTrue(northbound.getFeatureToggles().collectGrpcStats,
"This test is skipped because 'collectGrpcStats' is disabled")
        expect: "At least 1 result in the past 15 minutes"
        assert !statsMap.get(sw.getDpid()).get(metric).getDataPoints().isEmpty()

        where:
        [metric, sw] << ([[PACKET_IN_TOTAL_PACKETS,
                         ...
                         PACKET_OUT_ETH_0], getNoviflowSwitches()].combinations())
    }
