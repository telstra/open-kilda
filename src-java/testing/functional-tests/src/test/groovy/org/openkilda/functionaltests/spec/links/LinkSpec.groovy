...
class LinkSpec extends HealthCheckSpecification {
    ...
    def "Reroute all flows going through a particular link"() {
        ...
        availablePaths[1..-1].each { makePathIslsMorePreferable(preferablePath, it, northbound.getAllLinks()) }
        ...
        availablePaths[1..-1].each { makePathIslsMorePreferable(it, preferablePath, northbound.getAllLinks()) }
    ...
    def "Able to update max bandwidth for a link"() {
        ...
        [isl, isl.reversed].each {
            def linkDetails = it.getInfo(links)
