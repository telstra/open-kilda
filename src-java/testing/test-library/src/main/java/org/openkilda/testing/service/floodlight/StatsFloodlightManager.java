...
@Component("statsFloodlight")
public class StatsFloodlightManager implements MultiFloodlightManager {
    @Value("#{'${floodlight.regions}'.split(',')}")
    private List<String> regions;
    @Value("#{'${floodlight.controllers.stat.containers}'.split(',')}")
    private List<String> statsContainers;
    @Value("#{'${floodlight.controllers.stat.openflow}'.split(',')}")
    private List<String> statControllers;
    @Autowired
    @Qualifier("statsFloodlights")
    ...
    public String getContainerName(String region) {
        return statsContainers.get(getRegionIndex(region));
    }
