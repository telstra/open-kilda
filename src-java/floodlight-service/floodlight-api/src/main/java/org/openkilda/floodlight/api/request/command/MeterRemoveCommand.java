...
@Getter
public class MeterRemoveCommand extends MeterCommandBase {
    public MeterRemoveCommand(MessageContext messageContext, MeterSpeakerCommandData payload) {
        super(messageContext, payload);
    }

    @Override
    public CompletableFuture<MessageContext> execute(OfSpeaker speaker) {
        return speaker.removeMeter(getMessageContext(), getSwitchId(), meterData.getMeterId());
    }
}
