...
public class PingService implements IService {
    ...
    public IPacket wrapData(Ping ping, byte[] payload) {
        ...
        for (int vlanID : vlanStack) {
            l2Payload = injectVlan(l2Payload, vlanID, l2Type);
    ...
    private IPacket injectVlan(IPacket payload, int vlanId, EthType type) {
    ...
    private IPacket extractEthernetPayload(Ethernet packet, List<Integer> vlanStack) {
        // FIXME: remove code duplication (org.openkilda.floodlight.service.connected.ConnectedDevicesService)
