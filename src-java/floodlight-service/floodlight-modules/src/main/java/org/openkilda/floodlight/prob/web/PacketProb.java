...
public class PacketProb extends ServerResource {
    ...
    @Post("json")
    @Put("json")
    public String sendPacketProb(String json) throws JsonProcessingException {
        ProbResult result = new ProbResult(true, "ok");
        IProbService pvs =
                (IProbService) getContext().getAttributes()
                        .get(IProbService.class.getCanonicalName());
        try {
            PacketData request = MAPPER.readValue(json, PacketData.class);
            pvs.sendPacketProb(request);
        } catch (Exception e) {
