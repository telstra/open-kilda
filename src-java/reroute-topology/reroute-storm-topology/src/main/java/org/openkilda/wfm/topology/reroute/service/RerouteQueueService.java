...
public class RerouteQueueService {
    ...
    public void flushThrottling() {
        ...
        reroutes.forEach((flowId, rerouteQueue) -> {
            FlowThrottlingData flowThrottlingData = rerouteQueue.flushThrottling();
            if (flowThrottlingData != null) {
                requestsToSend.put(flowId, flowThrottlingData);
            }
        });
        ...
        requestsToSend.entrySet().stream()
                ...
                .forEach(es -> {
                    ...
                    String correlationId = new CommandContext(flowThrottlingData.getCorrelationId())
                            .fork(flowId).getCorrelationId();
                    flowThrottlingData.setCorrelationId(correlationId);
    ...
    public void handleTimeout(String correlationId) {
        log.warn("Reroute retry with correlation id {} timed out.", correlationId);
    ...
    private RerouteQueue getRerouteQueue(String flowId) {
        ...
        if (rerouteQueue == null) {
            rerouteQueue = RerouteQueue.empty();
