...
public abstract class YFlowMapper {
    ...
    public SubFlowDto toSubFlowDto(YSubFlow subFlow) {
        ...
        Optional<Flow> flow = Optional.ofNullable(subFlow.getSubFlow());
        return SubFlowDto.builder()
                .flowId(subFlow.getSubFlowId())
                .endpoint(endpoint)
                .sharedEndpoint(sharedEndpoint)
                .status(flow.map(Flow::getStatus).map(this::map).orElse(null))
                .description(flow.map(Flow::getDescription).orElse(null))
                .timeCreate(flow.map(Flow::getTimeCreate).map(this::map).orElse(null))
                .timeUpdate(flow.map(Flow::getTimeModify).map(this::map).orElse(null))
                .build();
    }

    public abstract String map(FlowStatus status);

    public abstract String map(Instant time);
}
