...
public class OnReceivedRemoveResponseAction extends
        BaseSpeakerResponseProcessingAction<YFlowRerouteFsm, State, Event, YFlowRerouteContext> {
    ...
    public void perform(State from, State to, Event event, YFlowRerouteContext context, YFlowRerouteFsm stateMachine) {
        ...
        if (stateMachine.getPendingCommands().isEmpty()) {
            if (stateMachine.getFailedCommands().isEmpty()) {
                log.debug("Received responses for all pending remove commands of the y-flow {}",
