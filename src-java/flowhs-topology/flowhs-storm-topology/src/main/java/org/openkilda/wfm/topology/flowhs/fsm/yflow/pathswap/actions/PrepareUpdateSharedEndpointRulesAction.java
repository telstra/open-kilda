...
@Slf4j
public class PrepareUpdateSharedEndpointRulesAction extends
        YFlowRuleManagerProcessingAction<YFlowPathSwapFsm, State, Event, YFlowPathSwapContext> {
    public PrepareUpdateSharedEndpointRulesAction(PersistenceManager persistenceManager, RuleManager ruleManager) {
    ...
    protected void perform(State from, State to, Event event, YFlowPathSwapContext context,
        ...
        transactionManager.doInTransaction(() -> {
            ...
            List<OfCommand> deleteOldYFlowOfCommands = buildFlowOnlyOfCommands(sharedEndpoint,
                    stateMachine.getOldPrimaryPaths());
            stateMachine.setDeleteOldYFlowOfCommands(deleteOldYFlowOfCommands);
        });
    }
}
