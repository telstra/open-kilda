        ...
        transactionManager.doInTransaction(() -> {
            ...
            subFlows.forEach(subFlow -> {
                ...
                if (!subFlow.isAllocateProtectedPath() || subFlow.getProtectedForwardPathId() == null
                    ...
                    throw new FlowProcessingException(ErrorType.REQUEST_INVALID,
                            format("Could not swap y-flow paths: sub-flow %s doesn't have protected path",
                                    subFlow.getFlowId()));
                }
                if (subFlow.getProtectedForwardPath().getStatus() != FlowPathStatus.ACTIVE
                    ...
                    throw new FlowProcessingException(ErrorType.REQUEST_INVALID,
                            format("Could not swap y-flow paths: protected path of sub-flow %s is not in ACTIVE state, "
                                            + "but in %s/%s state",
