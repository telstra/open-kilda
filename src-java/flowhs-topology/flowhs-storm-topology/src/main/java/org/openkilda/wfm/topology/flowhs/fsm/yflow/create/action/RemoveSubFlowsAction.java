...
public class RemoveSubFlowsAction extends HistoryRecordingAction<YFlowCreateFsm, State, Event, YFlowCreateContext> {
    ...
    public void perform(State from, State to, Event event, YFlowCreateContext context, YFlowCreateFsm stateMachine) {
        log.debug("Start removing {} sub-flows of y-flow {}", stateMachine.getSubFlows().size(),
                stateMachine.getYFlowId());
        stateMachine.clearPendingSubFlows();

        stateMachine.getSubFlows().forEach(flowId -> {
            CommandContext flowContext = stateMachine.getCommandContext().fork(flowId);
            stateMachine.addPendingSubFlow(flowId);
            flowDeleteService.startFlowDeletion(flowId, flowContext, flowId, false);
        });
    }
}
