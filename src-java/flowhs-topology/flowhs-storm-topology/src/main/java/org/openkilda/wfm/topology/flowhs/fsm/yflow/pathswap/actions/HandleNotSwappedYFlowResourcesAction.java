...
@Slf4j
public class HandleNotSwappedYFlowResourcesAction extends
        YFlowProcessingWithHistorySupportAction<YFlowPathSwapFsm, State, Event, YFlowPathSwapContext> {
    public HandleNotSwappedYFlowResourcesAction(PersistenceManager persistenceManager) {
    ...
    protected void perform(State from, State to, Event event, YFlowPathSwapContext context,
        ...
        transactionManager.doInTransaction(() -> {
            ...
            stateMachine.saveActionToHistory(format("Failed to swap y-flow main and protected y-points: %s / %s",
                    yFlow.getYPoint(), yFlow.getProtectedPathYPoint()));
        });
    }
}
