...
@Slf4j
public class InstallYPointMeterAction extends YFlowProcessingAction<YFlowCreateFsm, State, Event, YFlowCreateContext> {
    public InstallYPointMeterAction(PersistenceManager persistenceManager) {
    ...
    protected void perform(State from, State to, Event event, YFlowCreateContext context, YFlowCreateFsm stateMachine) {
        ...
        //TODO: build and send a y-point meter install command
        stateMachine.saveActionToHistory("No need to install y-point meter");
        stateMachine.fire(Event.YPOINT_METER_INSTALLED);
    }
}
