...
@Slf4j
public class HandleNotSwappedSubFlowAction extends
        FlowProcessingWithHistorySupportAction<YFlowPathSwapFsm, State, Event, YFlowPathSwapContext> {
    public HandleNotSwappedSubFlowAction(PersistenceManager persistenceManager) {
    ...
    protected void perform(State from, State to, Event event, YFlowPathSwapContext context,
        ...
        stateMachine.saveErrorToHistory("Failed to swap sub-flow paths",
                format("Failed to swap paths on sub-flow %s of y-flow %s: %s", subFlowId, stateMachine.getYFlowId(),
                        context.getError()));
        ...
        if (stateMachine.getSwappingSubFlows().isEmpty()) {
            stateMachine.fire(Event.FAILED_TO_SWAP_SUB_FLOWS);
            stateMachine.setErrorReason(format("Failed to swap paths for %s sub-flows of y-flow %s",
