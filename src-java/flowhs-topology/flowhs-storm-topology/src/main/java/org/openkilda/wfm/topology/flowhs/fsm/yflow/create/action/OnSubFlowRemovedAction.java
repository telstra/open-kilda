...
@Slf4j
public class OnSubFlowRemovedAction extends HistoryRecordingAction<YFlowCreateFsm, State, Event, YFlowCreateContext> {
    @Override
    protected void perform(State from, State to, Event event, YFlowCreateContext context, YFlowCreateFsm stateMachine) {
        ...
        stateMachine.saveActionToHistory("Removed a sub-flow",
                format("Removed sub-flow %s of y-flow %s", subFlowId, stateMachine.getYFlowId()));
