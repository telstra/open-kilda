    ...
    public void perform(State from, State to, Event event, YFlowCreateContext context, YFlowCreateFsm stateMachine) {
        ...
        if (response.isSuccess()) {
            ...
            if (attempt <= speakerCommandRetriesLimit
                ...
                //TODO: stateMachine.getCarrier().sendSpeakerRequest(command.makeValidateRequest(commandId));
            } else {
        ...
        if (stateMachine.getPendingCommands().isEmpty()) {
            if (stateMachine.getFailedValidationResponses().isEmpty()) {
                log.debug("Received responses for all pending validation commands of y-flow {}",
