...
import org.openkilda.wfm.topology.flowhs.bolts.HaFlowPathSwapHubBolt;
import org.openkilda.wfm.topology.flowhs.bolts.HaFlowPathSwapHubBolt.HaFlowPathSwapConfig;
...
import org.openkilda.wfm.topology.flowhs.bolts.HaFlowRerouteHubBolt;
import org.openkilda.wfm.topology.flowhs.bolts.HaFlowRerouteHubBolt.HaFlowRerouteConfig;
import org.openkilda.wfm.topology.flowhs.bolts.HaFlowSyncHubBolt;
import org.openkilda.wfm.topology.flowhs.bolts.HaFlowSyncHubBolt.HaFlowSyncConfig;
...
public class FlowHsTopology extends AbstractTopology<FlowHsTopologyConfig> {
    ...
    public StormTopology createTopology() {
        ...
        haFlowSyncHub(tb, persistenceManager);
        haFlowRerouteHub(tb, persistenceManager);
        haFlowReadBolt(tb, persistenceManager);
        haFlowValidationHub(tb, persistenceManager);
        haFlowPathSwapHub(tb, persistenceManager);

        speakerSpout(tb);
        speakerWorkers(tb);
        validationSpeakerWorkerForDumpsBolt(tb);
        speakerOutput(tb);
    ...
    private void zkBolt(TopologyBuilder topologyBuilder) {
        ZooKeeperBolt zooKeeperBolt = new ZooKeeperBolt(getConfig().getBlueGreenMode(), getZkTopoName(),
                ...
                getBoltInstancesCount(ComponentId.FLOW_CREATE_HUB.name(), ComponentId.FLOW_UPDATE_HUB.name(),
                        ...
                        ComponentId.HA_FLOW_REROUTE_HUB.name(),
                        ComponentId.HA_FLOW_DELETE_HUB.name(),
                        ComponentId.HA_FLOW_SYNC_HUB.name(),
                        ComponentId.HA_FLOW_READ_BOLT.name(),
                        ComponentId.HA_FLOW_PATH_SWAP_HUB.name(),
        ...
        declareBolt(topologyBuilder, zooKeeperBolt, ZooKeeperBolt.BOLT_ID)
                ...
                .allGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), ZkStreams.ZK.toString())
                .allGrouping(ComponentId.HA_FLOW_DELETE_HUB.name(), ZkStreams.ZK.toString())
                .allGrouping(ComponentId.HA_FLOW_SYNC_HUB.name(), ZkStreams.ZK.toString())
                .allGrouping(ComponentId.HA_FLOW_READ_BOLT.name(), ZkStreams.ZK.toString())
                .allGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), ZkStreams.ZK.toString())
    ...
    private void yFlowValidationHub(TopologyBuilder topologyBuilder, PersistenceManager persistenceManager) {
        ...
        declareBolt(topologyBuilder, hubBolt, ComponentId.YFLOW_VALIDATION_HUB.name())
                ...
                .directGrouping(ComponentId.FLOW_VALIDATION_SPEAKER_WORKER.name(),
                        SPEAKER_WORKER_TO_HUB_VALIDATION.name())
                .allGrouping(ZooKeeperSpout.SPOUT_ID)
                .directGrouping(CoordinatorBolt.ID);
    }
    ...
    private void haFlowRerouteHub(TopologyBuilder topologyBuilder, PersistenceManager persistenceManager) {
        int hubTimeout = (int) TimeUnit.SECONDS.toMillis(topologyConfig.getRerouteHubTimeoutSeconds());

        HaFlowRerouteConfig config = HaFlowRerouteConfig.builder()
                .pathAllocationRetriesLimit(topologyConfig.getPathAllocationRetriesLimit())
                .pathAllocationRetryDelay(topologyConfig.getPathAllocationRetryDelay())
                .speakerCommandRetriesLimit(topologyConfig.getRerouteSpeakerCommandRetries())
                .resourceAllocationRetriesLimit(topologyConfig.getResourceAllocationRetriesLimit())
        ...
        FlowResourcesConfig flowResourcesConfig = configurationProvider.getConfiguration(FlowResourcesConfig.class);
        RuleManagerConfig ruleManagerConfig = configurationProvider.getConfiguration(RuleManagerConfig.class);
        HaFlowRerouteHubBolt hubBolt = new HaFlowRerouteHubBolt(config, persistenceManager, pathComputerConfig,
                flowResourcesConfig, ruleManagerConfig);
        declareBolt(topologyBuilder, hubBolt, ComponentId.HA_FLOW_REROUTE_HUB.name())
                .fieldsGrouping(ComponentId.FLOW_ROUTER_BOLT.name(), ROUTER_TO_HA_FLOW_REROUTE_HUB.name(), FLOW_FIELD)
                .directGrouping(ComponentId.SPEAKER_WORKER.name(), SPEAKER_WORKER_TO_HUB.name())
                .allGrouping(ZooKeeperSpout.SPOUT_ID)
                .directGrouping(CoordinatorBolt.ID);
    }
    ...
    private void haFlowSyncHub(TopologyBuilder topologyBuilder, PersistenceManager persistenceManager) {
        int hubTimeout = (int) TimeUnit.SECONDS.toMillis(topologyConfig.getSyncHubTimeoutSeconds());

        HaFlowSyncConfig config = HaFlowSyncConfig.builder()
                .speakerCommandRetriesLimit(topologyConfig.getCreateSpeakerCommandRetries())
                .autoAck(true)
                .timeoutMs(hubTimeout)
                .requestSenderComponent(ComponentId.FLOW_ROUTER_BOLT.name())
        ...
        RuleManagerConfig ruleManagerConfig = configurationProvider.getConfiguration(RuleManagerConfig.class);
        HaFlowSyncHubBolt hubBolt = new HaFlowSyncHubBolt(config, persistenceManager, ruleManagerConfig);
        declareBolt(topologyBuilder, hubBolt, ComponentId.HA_FLOW_SYNC_HUB.name())
                .fieldsGrouping(ComponentId.FLOW_ROUTER_BOLT.name(),
                        Stream.ROUTER_TO_HA_FLOW_SYNC_HUB.name(), FLOW_FIELD)
                .directGrouping(ComponentId.SPEAKER_WORKER.name(), SPEAKER_WORKER_TO_HUB.name())
                .allGrouping(ZooKeeperSpout.SPOUT_ID)
                .directGrouping(CoordinatorBolt.ID);
    }
    ...
    private void haFlowPathSwapHub(TopologyBuilder topologyBuilder, PersistenceManager persistenceManager) {
        int hubTimeout = (int) TimeUnit.SECONDS.toMillis(topologyConfig.getPathSwapHubTimeoutSeconds());

        HaFlowPathSwapConfig config = HaFlowPathSwapConfig.builder()
                .speakerCommandRetriesLimit(topologyConfig.getPathSwapSpeakerCommandRetries())
                .autoAck(true)
                .timeoutMs(hubTimeout)
                .requestSenderComponent(ComponentId.FLOW_ROUTER_BOLT.name())
        ...
        RuleManagerConfig ruleManagerConfig = configurationProvider.getConfiguration(RuleManagerConfig.class);
        HaFlowPathSwapHubBolt hubBolt = new HaFlowPathSwapHubBolt(config, persistenceManager, ruleManagerConfig);
        declareBolt(topologyBuilder, hubBolt, ComponentId.HA_FLOW_PATH_SWAP_HUB.name())
                .fieldsGrouping(ComponentId.FLOW_ROUTER_BOLT.name(), ROUTER_TO_HA_FLOW_PATH_SWAP_HUB.name(), FLOW_FIELD)
                .directGrouping(ComponentId.SPEAKER_WORKER.name(), SPEAKER_WORKER_TO_HUB.name())
                .allGrouping(ZooKeeperSpout.SPOUT_ID)
                .directGrouping(CoordinatorBolt.ID);
    }
    ...
    private void speakerWorkers(TopologyBuilder topologyBuilder) {
        ...
        SpeakerWorkerBolt speakerWorker = new SpeakerWorkerBolt(Config.builder()
                ...
                .hubComponent(ComponentId.HA_FLOW_REROUTE_HUB.name())
                .hubComponent(ComponentId.HA_FLOW_DELETE_HUB.name())
                .hubComponent(ComponentId.HA_FLOW_SYNC_HUB.name())
                .hubComponent(ComponentId.HA_FLOW_PATH_SWAP_HUB.name())
        ...
        declareBolt(topologyBuilder, speakerWorker, ComponentId.SPEAKER_WORKER.name())
                ...
                .fieldsGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), Stream.HUB_TO_SPEAKER_WORKER.name(), FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_DELETE_HUB.name(), Stream.HUB_TO_SPEAKER_WORKER.name(), FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_SYNC_HUB.name(), Stream.HUB_TO_SPEAKER_WORKER.name(), FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), Stream.HUB_TO_SPEAKER_WORKER.name(),
                        FIELDS_KEY)
                .directGrouping(CoordinatorBolt.ID);
    }

    private void validationSpeakerWorkerForDumpsBolt(TopologyBuilder topologyBuilder) {
        int speakerTimeout = (int) TimeUnit.SECONDS.toMillis(topologyConfig.getValidationSpeakerTimeoutSeconds());
        SpeakerWorkerForDumpsBolt speakerWorkerForDumpsBolt = new SpeakerWorkerForDumpsBolt(Config.builder()
                .defaultTimeout(speakerTimeout)
                .workerSpoutComponent(ComponentId.SPEAKER_WORKER_SPOUT.name())
        ...
        declareBolt(topologyBuilder, speakerWorkerForDumpsBolt, ComponentId.FLOW_VALIDATION_SPEAKER_WORKER.name())
                .fieldsGrouping(ComponentId.SPEAKER_WORKER_SPOUT.name(), FIELDS_KEY)
                .fieldsGrouping(ComponentId.FLOW_VALIDATION_HUB.name(),
                        Stream.HUB_TO_SPEAKER_WORKER.name(), FIELDS_KEY)
                .fieldsGrouping(ComponentId.YFLOW_VALIDATION_HUB.name(),
                        Stream.HUB_TO_SPEAKER_WORKER.name(), FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_VALIDATION_HUB.name(),
                        Stream.HUB_TO_SPEAKER_WORKER.name(), FIELDS_KEY)
                .directGrouping(CoordinatorBolt.ID);
    }
    ...
    private void coordinator(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, new CoordinatorBolt(), CoordinatorBolt.ID)
                ...
                .fieldsGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), CoordinatorBolt.INCOME_STREAM, FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_DELETE_HUB.name(), CoordinatorBolt.INCOME_STREAM, FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_SYNC_HUB.name(), CoordinatorBolt.INCOME_STREAM, FIELDS_KEY)
                .fieldsGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), CoordinatorBolt.INCOME_STREAM, FIELDS_KEY)
    ...
    private void northboundOutput(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, nbKafkaBolt, ComponentId.NB_RESPONSE_SENDER.name())
                ...
                .shuffleGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), Stream.HUB_TO_NB_RESPONSE_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_DELETE_HUB.name(), Stream.HUB_TO_NB_RESPONSE_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_SYNC_HUB.name(), Stream.HUB_TO_NB_RESPONSE_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_READ_BOLT.name())
                .shuffleGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), Stream.HUB_TO_NB_RESPONSE_SENDER.name())
    ...
    private void rerouteTopologyOutput(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, rerouteKafkaBolt, ComponentId.REROUTE_RESPONSE_SENDER.name())
                ...
                .shuffleGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(),
                        Stream.HUB_TO_REROUTE_RESPONSE_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), Stream.HUB_TO_REROUTE_RESPONSE_SENDER.name());
    }
    ...
    private void pingOutput(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, pingKafkaBolt, ComponentId.FLOW_PING_SENDER.name())
                ...
                .shuffleGrouping(ComponentId.HA_FLOW_CREATE_HUB.name(), Stream.HUB_TO_PING_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_UPDATE_HUB.name(), Stream.HUB_TO_PING_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_DELETE_HUB.name(), Stream.HUB_TO_PING_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), Stream.HUB_TO_PING_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), Stream.HUB_TO_PING_SENDER.name())
    ...
    private void flowMonitoringTopologyOutput(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, flowMonitoringKafkaBolt, ComponentId.FLOW_MONITORING_TOPOLOGY_SENDER.name())
                ...
                .shuffleGrouping(ComponentId.HA_FLOW_UPDATE_HUB.name(),
                        Stream.HUB_TO_FLOW_MONITORING_TOPOLOGY_SENDER.name())
                .shuffleGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(),
                        Stream.HUB_TO_FLOW_MONITORING_TOPOLOGY_SENDER.name())
    ...
    private void history(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, kafkaBolt, ComponentId.HISTORY_BOLT.name())
                ...
                .fieldsGrouping(
                        ComponentId.HA_FLOW_REROUTE_HUB.name(), Stream.HUB_TO_HISTORY_TOPOLOGY_SENDER.name(), grouping)
                .fieldsGrouping(
                        ComponentId.HA_FLOW_DELETE_HUB.name(), Stream.HUB_TO_HISTORY_TOPOLOGY_SENDER.name(), grouping)
                .fieldsGrouping(
                        ComponentId.HA_FLOW_SYNC_HUB.name(), Stream.HUB_TO_HISTORY_TOPOLOGY_SENDER.name(), grouping)
                .fieldsGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), Stream.HUB_TO_HISTORY_TOPOLOGY_SENDER.name(),
                        grouping);
    }
    ...
    private void metrics(TopologyBuilder topologyBuilder) {
        ...
        declareBolt(topologyBuilder, kafkaBolt, ComponentId.METRICS_BOLT.name())
                ...
                .shuffleGrouping(ComponentId.HA_FLOW_REROUTE_HUB.name(), Stream.HUB_TO_METRICS_BOLT.name())
                .shuffleGrouping(ComponentId.HA_FLOW_DELETE_HUB.name(), Stream.HUB_TO_METRICS_BOLT.name())
                .shuffleGrouping(ComponentId.HA_FLOW_SYNC_HUB.name(), Stream.HUB_TO_METRICS_BOLT.name())
                .shuffleGrouping(ComponentId.HA_FLOW_PATH_SWAP_HUB.name(), Stream.HUB_TO_METRICS_BOLT.name())
    ...
    public enum Stream {
        ...
        ROUTER_TO_HA_FLOW_REROUTE_HUB,
        ROUTER_TO_HA_FLOW_DELETE_HUB,
        ROUTER_TO_HA_FLOW_READ,
        ROUTER_TO_HA_FLOW_VALIDATION_HUB,
        ROUTER_TO_HA_FLOW_PATH_SWAP_HUB,
        ROUTER_TO_HA_FLOW_SYNC_HUB,
