...
export class SwitchStoreComponent implements OnInit {
    ...
    this.storesettingservice.getSwitchStoreDetails().subscribe(
      jsonResponse => {
        if (
          ...
          const response = {urls: {...newResponse, ...jsonResponse.urls}};
  ...
  submitSwitchStore() {
    ...
    const obj = this.switchStoreForm.value;
    ...
    modalReff.result.then((response) => {
      if (response && response == true) {
        ...
        this.storesettingservice.deleteSwitchStore('/store/switch-store-config/delete').subscribe((res: any) => {
          ...
          const errorMsg = error && error.error && error.error['error-auxiliary-message'] ? error.error['error-auxiliary-message'] : MessageObj.error_in_deleting_switch_store;
          this.toastr.error(errorMsg, 'Error');
        });
      }
    });
