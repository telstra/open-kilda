...
export class SwitchDatatableComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  exportCsv(val) {
    ...
    this.data.forEach(function(d) {
      ...
      if (!val) {
        const locationString = 'longitude:' + ((d.location.longitude) ? d.location.longitude : '-') + ', latitude:' + ((d.location.latitude) ? d.location.latitude : '-') + ', city:' + ((d.location.city) ? d.location.city : '-') + ', street:' + ((d.location.street) ? d.location.street : '-') + ', Country:' + ((d.location.country) ? d.location.country : '-');
