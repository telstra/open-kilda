...
import { FlowsService } from 'src/app/common/services/flows.service';
  ...
  islMaintenance(e) {
    ...
    modalRef.componentInstance.emitService.subscribe(
      (evacuate: any) => {
        const data = {src_switch: this.src_switch, src_port: this.src_port, dst_switch: this.dst_switch, dst_port: this.dst_port, under_maintenance: e.target.checked, evacuate: evacuate.evaluateValue};
    ...
    if (this.enable_bfd) {
     ...
     modalRef.result.then((response) => {
      if (response && response == true) {
        const data = {src_switch: this.src_switch, src_port: this.src_port, dst_switch: this.dst_switch, dst_port: this.dst_port, enable_bfd: this.enable_bfd};
  ...
  evacuateIsl(e) {
    ...
    if (this.evacuate) {
     ...
     modalRef.result.then((response) => {
      if (response && response == true) {
        const data = {src_switch: this.src_switch, src_port: this.src_port, dst_switch: this.dst_switch, dst_port: this.dst_port, under_maintenance: this.under_maintenance, evacuate: e.target.checked};
        this.islListService.islUnderMaintenance(data).subscribe(response => {
...
get f() {
  ...
  CallFlowGraphAPI() {
    ...
    if (formdata.timezone == 'UTC') {
     ...
     const requestForwardPayload = {switches: [this.src_switch_kilda], outPort: this.src_port, startdate: convertedStartDate, enddate: convertedEndDate, downsample: downsampling, direction: direction, metric: metric};
     const requestReversePayload = { switches: [this.dst_switch_kilda], inPort: this.dst_port, startdate: convertedStartDate, enddate: convertedEndDate, downsample: downsampling, direction: direction, metric: metric};
      this.graphLoader.show(this.loaderName);
     this.flowService.getFlowPathStats(requestForwardPayload).subscribe((dataForward: any) => {
      this.flowService.getFlowPathStats(requestReversePayload).subscribe((dataReverse: any) => {
            const data = dataForward.concat(dataReverse);
