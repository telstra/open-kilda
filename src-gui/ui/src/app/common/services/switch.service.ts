...
export class SwitchService {
    ...
    getSwitchDetails(switchId, filter): Observable<any[]> {
        const reqParams: any = {};
        if (switchId) {
            reqParams.switchId = switchId;
        }
        reqParams.controller = filter === 'controller';
        return this.httpClient.get<any[]>(`${environment.apiEndPoint}/switch/details`, {params: reqParams});
    }
    ...
    hasDiscrepancy(switchDetail: any): boolean {
        if (!switchDetail.inventory_switch_detail) {
            return true;
        }
        if (switchDetail.switch_id == null) {
            return true;
        }
        return switchDetail.state !== switchDetail.inventory_switch_detail.status;
    }

    extractSwitchId(switchDetail: any): string {
        return switchDetail.switch_id || switchDetail.inventory_switch_detail['switch-id'];
    }

    isControllerSwitch(switchDetail: any): boolean {
        return !!switchDetail.switch_id;
    }

    isInventorySwitch(switchDetail: any): boolean {
        return !!switchDetail.inventory_switch_detail;
    }
