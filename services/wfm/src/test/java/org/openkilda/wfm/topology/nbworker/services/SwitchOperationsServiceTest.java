...
public class SwitchOperationsServiceTest extends Neo4jBasedTest {
    ...
    @Test
    public void shouldCreatePortProperties() {
        Switch sw = Switch.builder().switchId(TEST_SWITCH_ID).status(SwitchStatus.ACTIVE).build();
        switchRepository.createOrUpdate(sw);
        int port = 7;

        switchOperationsService.createOrUpdatePortProperties(TEST_SWITCH_ID, port, true);

        PortProperties actual = portPropertiesRepository.getBySwitchIdAndPort(TEST_SWITCH_ID, port).get();
        assertTrue(actual.isDiscoveryEnabled());
    }

    @Test
    public void shouldUpdatePortProperties() {
        Switch sw = Switch.builder().switchId(TEST_SWITCH_ID).status(SwitchStatus.ACTIVE).build();
        switchRepository.createOrUpdate(sw);
        int port = 7;
