...
public class RerouteServiceTest {
    ...
    @Test
    public void testGetAffectedUnpinnedFlows() {
        RepositoryFactory repositoryFactory = mock(RepositoryFactory.class);
        FlowRepository flowRepository = mock(FlowRepository.class);
        when(flowRepository.findActiveUnpinnedFlowsWithPortInPath(SWITCH_ID_A, PORT))
                .thenReturn(Collections.singletonList(unpinnedFlow));
        when(repositoryFactory.createFlowRepository()).thenReturn(flowRepository);

        RerouteService rerouteService = new RerouteService(repositoryFactory);
        Collection<Flow> flows = rerouteService.getAffectedUnpinnedFlows(SWITCH_ID_A, PORT);
        assertTrue(flows.contains(unpinnedFlow));
    }

    @Test
    public void testGetAffectedPinnedFlows() {
        RepositoryFactory repositoryFactory = mock(RepositoryFactory.class);
        FlowRepository flowRepository = mock(FlowRepository.class);
        when(flowRepository.findActivePinnedFlowsWithPortInPath(SWITCH_ID_A, PORT))
                .thenReturn(Collections.singletonList(pinnedFlow));
        when(repositoryFactory.createFlowRepository()).thenReturn(flowRepository);

        RerouteService rerouteService = new RerouteService(repositoryFactory);
        Collection<Flow> flows = rerouteService.getAffectedPinnedFlows(SWITCH_ID_A, PORT);
        assertTrue(flows.contains(pinnedFlow));
    }

    @Test
    public void testRerouteAffectedFlowsOnlyUnpinned() {
        RepositoryFactory repositoryFactory = mock(RepositoryFactory.class);
        FlowRepository flowRepository = mock(FlowRepository.class);
        when(flowRepository.findActiveUnpinnedFlowsWithPortInPath(SWITCH_ID_A, PORT))
                .thenReturn(Collections.singletonList(pinnedFlow));
        ...
        RerouteService rerouteService = new RerouteService(repositoryFactory);
        rerouteService.rerouteAffectedFlows(messageSender, CORRELATION_ID,
                REROUTE_AFFECTED_FLOWS_COMMAND);
        verify(messageSender).sendRerouteCommand(CORRELATION_ID, pinnedFlow, REASON);
    }

    @Test
    public void testRerouteAffectedFlowsOnlyPinned() {
        RepositoryFactory repositoryFactory = mock(RepositoryFactory.class);
        FlowRepository flowRepository = mock(FlowRepository.class);
        when(flowRepository.findActiveUnpinnedFlowsWithPortInPath(SWITCH_ID_A, PORT))
                .thenReturn(Collections.emptyList());
        ...
        verify(messageSender, never()).sendRerouteCommand(CORRELATION_ID, unpinnedFlow, REASON);
    }

    @Test
    public void testRerouteAffectedFlowsMixedFlows() {
        RepositoryFactory repositoryFactory = mock(RepositoryFactory.class);
        FlowRepository flowRepository = mock(FlowRepository.class);
        when(flowRepository.findActiveUnpinnedFlowsWithPortInPath(SWITCH_ID_A, PORT))
                .thenReturn(Collections.singletonList(unpinnedFlow));
        ...
        rerouteService.rerouteAffectedFlows(messageSender, CORRELATION_ID,
                REROUTE_AFFECTED_FLOWS_COMMAND);
        verify(messageSender).sendRerouteCommand(CORRELATION_ID, unpinnedFlow, REASON);
        verify(flowRepository).createOrUpdate(pinnedFlow);
    }

    @Test
    public void testRerouteInactiveUnpinnedFlows() {
        unpinnedFlow.setStatus(FlowStatus.DOWN);
        RepositoryFactory repositoryFactory = mock(RepositoryFactory.class);
        FlowRepository flowRepository = mock(FlowRepository.class);
