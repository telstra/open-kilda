...
public class NetworkPortServiceTest {
    ...
    public void inUnOperationalUpDownPort() {
        ...
        verify(carrier).sendPortStateChangedHistory(eq(endpoint), eq(PortHistoryAction.PORT_UP), any(Instant.class));
        verify(carrier).sendPortStateChangedHistory(eq(endpoint), eq(PortHistoryAction.PORT_DOWN), any(Instant.class));
