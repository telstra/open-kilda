...
public class ValidationServiceTest {
    ...
    public void empty() {
        ValidationService validationService = new ValidationService(
                generateSwitchFlowEntries(),
                repositoryFactory().build()
        );
        SyncRulesResponse response = validationService.validate();
    ...
    public void simpleSegmentCookies() {
        ValidationService validationService = new ValidationService(
                generateSwitchFlowEntries(1L, 2L),
                repositoryFactory().withSegmentsCookies(2L, 3L).build()
        );
        SyncRulesResponse response = validationService.validate();
    ...
    public void segmentAndIngressCookies() {
        ValidationService validationService = new ValidationService(
                generateSwitchFlowEntries(1L, 2L),
                repositoryFactory().withSegmentsCookies(2L).withIngressCookies(1L).build()
        );
        SyncRulesResponse response = validationService.validate();
        assertTrue(response.getMissingRules().isEmpty());
    ...
    public void ignoreDefaulRules() {
        ValidationService validationService = new ValidationService(
                generateSwitchFlowEntries(0x8000000000000002L),
                repositoryFactory().build()
        );
        SyncRulesResponse response = validationService.validate();
        assertTrue(response.getMissingRules().isEmpty());
    ...
    private static class RepositoryFactoryBuilder {
        ...
        private RepositoryFactory build() {
            ...
            for (long cookie: segmentsCookies) {
                ...
                when(flowSegment.getCookieId()).thenReturn(cookie);
            ...
            when(switchRepository.findFlowsFromSwitch(any())).thenReturn(flows);
            when(switchRepository.findFlowSegmentsToSwitch(any())).thenReturn(flowSegments);
