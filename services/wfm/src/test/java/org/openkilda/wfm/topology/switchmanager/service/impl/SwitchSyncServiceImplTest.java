...
public class SwitchSyncServiceImplTest {
    ...
    public void setUp() {
        ...
        when(commandBuilder.buildCommandsToCreateMissingRules(eq(SWITCH_ID), any()))
    ...
    public void handleCommandBuilderMissingRulesException() {
        ...
        verify(commandBuilder).buildCommandsToCreateMissingRules(eq(SWITCH_ID), eq(missingRules));
        verify(carrier).endProcessing(eq(KEY));
