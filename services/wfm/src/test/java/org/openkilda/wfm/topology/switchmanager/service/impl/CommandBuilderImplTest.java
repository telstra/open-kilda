...
public class CommandBuilderImplTest {
    ...
    private static class PersistenceManagerBuilder {
        ...
        /*
        private Flow makeFlow(String flowId, SwitchId source, SwitchId dest) {
            return Flow.builder()
                    .flowId(flowId)
                    .srcSwitch(makeSwitch(source))
                    .destSwitch(makeSwitch(dest))
                    .build();
        }

        private FlowPath makeFlowPath(SwitchId source, Switch dest) {

        }

        private Switch makeSwitch(SwitchId id) {
            return switchStorage.computeIfAbsent(id, key -> Switch.builder().switchId(key).build());
        }
        */
    }
}
