...
public class CacheBolt extends AbstractBolt implements CacheCarrier {
    ...
    protected void handleInput(Tuple tuple) throws PipelineException {
        ...
        if (ISL_STATUS_SPOUT_ID.equals(tuple.getSourceComponent())) {
            ...
            long timestamp = pullValue(tuple, TIMESTAMP_FIELD, Long.class);
            cacheService.handleGetDataFromCache(roundTripLatency, timestamp);
        } else {
    ...
    @Override
    public void emitCachedData(IslRoundTripLatency data, Endpoint destination, long timestamp) {
