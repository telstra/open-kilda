...
import org.openkilda.wfm.topology.switchmanager.model.ValidateResult;
...
public class SwitchSyncRulesFsm
    ...
    protected void receiveRules(SwitchSyncRulesState from, SwitchSyncRulesState to,
        ...
        carrier.dumpRules(key, dumpCommandMessage);
    }
    ...
    protected void validateRules(SwitchSyncRulesState from, SwitchSyncRulesState to,
        ...
        try {
            ValidationService validationService = new ValidationServiceImpl(carrier.getPersistenceManager());
            validateResult = validationService.validate(switchId, presentCookies);

            if (validateResult.getMissingRules().isEmpty()) {
                fire(FINISH);
            }
        } catch (Exception e) {
            sendException(e);
    ...
    protected void computeInstallRules(SwitchSyncRulesState from, SwitchSyncRulesState to,
        ...
        try {
            CommandBuilder commandBuilder = new CommandBuilderImpl(carrier.getPersistenceManager());
            missingRules = commandBuilder.buildCommandsToSyncRules(switchId, validateResult.getMissingRules());
        } catch (Exception e) {
            sendException(e);
        }
    }
    ...
    protected void installRules(SwitchSyncRulesState from, SwitchSyncRulesState to,
        ...
        carrier.installRules(key, installCommandMessage);
    }
    ...
    protected void finished(SwitchSyncRulesState from, SwitchSyncRulesState to,
                            SwitchSyncRulesEvent event, Object context) {
        SyncRulesResponse response = new SyncRulesResponse(validateResult.getMissingRules(),
                validateResult.getProperRules(),
                validateResult.getExcessRules(),
                validateResult.getMissingRules());
