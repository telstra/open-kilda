...
public final class FlowRerouteFsm
    ...
    private Map<UUID, FlowSegmentRequestProxiedFactory> ingressCommands = new HashMap<>();
    private Map<UUID, FlowSegmentRequestProxiedFactory> nonIngressCommands = new HashMap<>();
    private Map<UUID, FlowSegmentRequestProxiedFactory> removeCommands = new HashMap<>();
    ...
    private static StateMachineBuilder<FlowRerouteFsm, State, Event, FlowRerouteContext> builder(
        ...
        builder.internalTransition().within(State.VALIDATING_INGRESS_RULES).on(Event.COMMAND_EXECUTED)
                .perform(new ConsiderIngressRulesValidationResultAction());
