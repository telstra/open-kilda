...
public abstract class AbstractTopology<T extends AbstractTopologyConfig> implements Topology {
    ...
    /**
     * Creates Kafka OTSDB spout. Transforms received value to {@link InfoData}.
     *
     * @param topic Kafka topic
     * @return {@link KafkaSpout}
     */
    protected KafkaSpout<String, InfoData> buildKafkaOtsdbSpout(String topic, String spoutId) {
        KafkaSpoutConfig<String, InfoData> config = getKafkaOtsdbSpoutConfigBuilder(topic, spoutId).build();
        logger.info("Setup kafka spout: id={}, group={}, subscriptions={}",
                spoutId, config.getConsumerGroupId(), config.getSubscription().getTopicsString());
        return new KafkaSpout<>(config);
    }
    ...
    protected KafkaSpoutConfig.Builder<String, InfoData> getKafkaOtsdbSpoutConfigBuilder(String topic, String spoutId) {
        KafkaSpoutConfig.Builder<String, InfoData> config = new KafkaSpoutConfig.Builder<>(kafkaConfig.getHosts(),
                StringDeserializer.class, InfoDataDeserializer.class, new CustomNamedSubscription(topic));
