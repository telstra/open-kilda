...
import org.openkilda.wfm.topology.network.storm.bolt.speaker.command.SpeakerFlowIslRulesInstalledCommand;
import org.openkilda.wfm.topology.network.storm.bolt.speaker.command.SpeakerFlowIslRulesRemovedCommand;
import org.openkilda.wfm.topology.network.storm.bolt.speaker.command.SpeakerFlowWorkerCommand;
...
@Slf4j
public class SpeakerFlowRouter extends AbstractBolt {
    public static final String BOLT_ID = ComponentId.SPEAKER_FLOW_ROUTER.toString();
    ...
    protected void handleInput(Tuple input) throws Exception {
        ...
        if (ComponentId.INPUT_SPEAKER_FLOW.toString().equals(source)) {
    ...
    private void proxySpeaker(Tuple input, InfoData payload) throws PipelineException {
        if (payload instanceof InstallIslDefaultRulesResult) {
            emit(STREAM_WORKER_ID, input, makeWorkerTuple(new SpeakerFlowIslRulesInstalledCommand(
            ...
            emit(STREAM_WORKER_ID, input, makeWorkerTuple(new SpeakerFlowIslRulesRemovedCommand(
    ...
    private Values makeWorkerTuple(SpeakerFlowWorkerCommand command) throws PipelineException {
