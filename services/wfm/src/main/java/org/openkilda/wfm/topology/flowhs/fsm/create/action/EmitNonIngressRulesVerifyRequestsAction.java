...
public class EmitNonIngressRulesVerifyRequestsAction
    ...
    public EmitNonIngressRulesVerifyRequestsAction(Builder speakerCommandFsmBuilder) {
    ...
    public void execute(State from, State to, Event event, FlowCreateContext context, FlowCreateFsm stateMachine) {
        ...
        final Map<UUID, SpeakerCommandObserver> pendingCommands = stateMachine.getPendingRequests();
        for (FlowSegmentRequestProxiedFactory factory : stateMachine.getNonIngressCommands().values()) {
            FlowSegmentRequest request = factory.makeVerifyRequest(commandIdGenerator.generate());
            // TODO ensure no conflicts
            pendingCommands.put(request.getCommandId(), new SpeakerCommandObserver(speakerCommandFsmBuilder, request));
        }
    }
}
