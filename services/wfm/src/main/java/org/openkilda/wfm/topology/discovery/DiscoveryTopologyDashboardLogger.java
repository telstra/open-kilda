...
public class DiscoveryTopologyDashboardLogger extends AbstractLogWrapper {
    ...
    public void onIslUpdateStatus(Level level, String message, IslReference ref, String state) {
        ...
        String srcSwitch = ref.getSource().getDatapath().toString();
        String dstSwitch = ref.getDest().getDatapath().toString();
        String srcPort = String.valueOf(ref.getSource().getPortNumber());
        String dstPort = String.valueOf(ref.getDest().getPortNumber());

        data.put(TAG, "switch-port-isl");
        data.put(TYPE, "isl");
        data.put(SRC_SWITCH, srcSwitch);
        data.put(DST_SWITCH, dstSwitch);
        data.put(STATE, state);
        data.put(SRC_PORT, srcPort);
        data.put(DST_PORT, dstPort);
        data.put(SRC_SWITCH_PORT, srcSwitch + "_" + srcPort);
        data.put(DST_SWITCH_PORT, dstSwitch + "_" + dstPort);
        proceed(level, message, data);
    }
