...
public class PathsService {
     ...
     * Get paths.
     */
    public PathsInfoData getPaths(SwitchId srcSwitchId, SwitchId dstSwitchId)
        ...
        List<Path> paths = flowPaths.stream().map(PathMapper.INSTANCE::map).collect(Collectors.toList());
        long totalBandwidth = flowPaths.stream().mapToLong(FlowPath::getMinAvailableBandwidth).sum();

        return new PathsInfoData(totalBandwidth, paths);
    }
}
