...
public class Sender implements ISender {
    ...
    @Override
    public void sendInfoToWfmDump(InfoData data, String correlationId) throws JsonProcessingException {
        Message message = new InfoMessage(data, System.currentTimeMillis(), correlationId, Destination.WFM);
        send(StreamType.WFM_DUMP.toString(), message);
    }

    @Override
    public void sendCommandToWfmDump(CommandData data, String correlationId) throws JsonProcessingException {
        Message message = new CommandMessage(data, System.currentTimeMillis(), correlationId, Destination.WFM);
        send(StreamType.WFM_DUMP.toString(), message);
    }
