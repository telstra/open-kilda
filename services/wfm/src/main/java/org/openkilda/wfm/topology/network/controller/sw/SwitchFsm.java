...
public final class SwitchFsm extends AbstractBaseFsm<SwitchFsm, SwitchFsmState, SwitchFsmEvent, SwitchFsmContext> {
    ...
    private final boolean isProcessMetersWhenSwitchSync;
    private final boolean isRemoveExcessWhenSwitchSync;
    private final int countSynchronizationAttempts;
    ...
    public SwitchFsm(PersistenceManager persistenceManager, SwitchId switchId, NetworkOptions options) {
        ...
        this.isProcessMetersWhenSwitchSync = options.isProcessMetersWhenSwitchSync();
        this.isRemoveExcessWhenSwitchSync = options.isRemoveExcessWhenSwitchSync();
        this.countSynchronizationAttempts = options.getCountSynchronizationAttempts();
    }
    ...
    public void synchronizeEnter(SwitchFsmState from, SwitchFsmState to, SwitchFsmEvent event,
        ...
        syncAttempts = countSynchronizationAttempts;
    ...
    private boolean excessRulesCheck(RulesSyncEntry rules) {
        return !isRemoveExcessWhenSwitchSync
    ...
    private boolean missingMetersCheck(MetersSyncEntry meters) {
        return !isProcessMetersWhenSwitchSync
                || meters.getMissing().isEmpty() || meters.getInstalled().containsAll(meters.getMissing());
    }

    private boolean excessMetersCheck(MetersSyncEntry meters) {
        return !isRemoveExcessWhenSwitchSync && !isProcessMetersWhenSwitchSync
    ...
    public static class SwitchFsmFactory {
        ...
        SwitchFsmFactory() {
            ...
            builder.transition()
                    .from(SwitchFsmState.INIT).to(SwitchFsmState.SYNCHRONIZE).on(SwitchFsmEvent.ONLINE);

            // SYNCHRONIZE
            builder.internalTransition().within(SwitchFsmState.SYNCHRONIZE).on(SwitchFsmEvent.SYNCHRONIZE)
                    .callMethod("synchronizeSwitch");
            builder.internalTransition().within(SwitchFsmState.SYNCHRONIZE).on(SwitchFsmEvent.RECEIVED_SYNC_RESPONSE)
                    .callMethod("switchSynchronized");
            builder.internalTransition()
                    .within(SwitchFsmState.SYNCHRONIZE).on(SwitchFsmEvent.RECEIVED_SYNC_ERROR_RESPONSE)
                    .callMethod("switchSynchronizedError");
            builder.internalTransition()
                    .within(SwitchFsmState.SYNCHRONIZE).on(SwitchFsmEvent.RECEIVED_SYNC_TIMEOUT_RESPONSE)
                    .callMethod("switchSynchronizedTimeout");
            builder.transition()
                    .from(SwitchFsmState.SYNCHRONIZE).to(SwitchFsmState.SETUP).on(SwitchFsmEvent.SYNCHRONIZED);
            builder.onEntry(SwitchFsmState.SYNCHRONIZE)
            ...
            // OFFLINE
            builder.transition().from(SwitchFsmState.OFFLINE).to(SwitchFsmState.SYNCHRONIZE).on(SwitchFsmEvent.ONLINE);
    ...
    public enum SwitchFsmEvent {
        ...
        SYNCHRONIZE,
        SYNCHRONIZED,
        RECEIVED_SYNC_RESPONSE,
        RECEIVED_SYNC_ERROR_RESPONSE,
        RECEIVED_SYNC_TIMEOUT_RESPONSE,
