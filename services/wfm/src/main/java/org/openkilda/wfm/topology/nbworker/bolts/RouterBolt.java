...
public class RouterBolt extends AbstractBolt {
    ...
    private void processRequest(Tuple input, BaseRequest request, CommandContext context) {
        if (request instanceof SwitchesBaseRequest) {
            getOutput().emit(StreamType.SWITCH.toString(), input, new Values(request, context));
        } else if (request instanceof LinksBaseRequest) {
            getOutput().emit(StreamType.ISL.toString(), input, new Values(request, context));
        } else if (request instanceof FlowsBaseRequest) {
            getOutput().emit(StreamType.FLOW.toString(), input, new Values(request, context));
        } else if (request instanceof FeatureTogglesBaseRequest) {
            getOutput().emit(StreamType.FEATURE_TOGGLES.toString(), input, new Values(request, context));
        } else if (request instanceof GetPathsRequest) {
            getOutput().emit(StreamType.PATHS.toString(), input, new Values(request, context));
        } else if (request instanceof GetFlowHistoryRequest) {
            getOutput().emit(StreamType.HISTORY.toString(), input, new Values(request, context));
        } else {
            unhandledInput(input);
        }
    }

    private void processRequest(Tuple input, InfoData data, CommandContext context) {
        if (data instanceof SwitchFlowEntries) {
            getOutput().emit(StreamType.VALIDATION.toString(), input, new Values(data, context));
        } else {
            unhandledInput(input);
        }
    }

    private void processRequest(Tuple input, ErrorData data, CommandContext context) {
        getOutput().emit(StreamType.ERROR.toString(), input, new Values(data, context));
    }
