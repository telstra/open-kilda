...
public final class PortReportFsm extends AbstractStateMachine<PortReportFsm, PortFsmState, PortFsmEvent, Void> {
    ...
    public void becomeDown(PortFsmState from, PortFsmState to, PortFsmEvent event) {
        ...
        sendPortHistory(false, carrier);
    }

    private void sendPortHistory(boolean isUp, IPortCarrier carrier) {
        Instant time = Instant.now();
        carrier.sendPortHistory(PortHistoryData.builder()
                .endpoint(endpoint)
                .status(isUp ? PortStatus.UP : PortStatus.DOWN)
                .bouncingStarted(time)
                .bouncingEnded(time)
                .build());
    }
