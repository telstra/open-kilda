    ...
    void checkFlowForEndpointConflicts(Flow requestedFlow) throws FlowValidationException {
        ...
        if (conflictSrcSrc.isPresent()) {
            String errorMessage = format("Requested flow '%s' source conflicts with flow '%s' source. "
                    ...
                    requestedFlow.getFlowId(), requestedFlow.getSrcSwitch(),
                    requestedFlow.getSrcPort(), requestedFlow.getSrcVlan(),
                    conflictSrcSrc.get().getFlowId(), conflictSrcSrc.get().getSrcSwitch(),
        ...
        if (conflictDstSrc.isPresent()) {
            String errorMessage = format("Requested flow '%s' source conflicts with flow '%s' source. "
                    ...
                    requestedFlow.getFlowId(), requestedFlow.getSrcSwitch(),
                    requestedFlow.getSrcPort(), requestedFlow.getSrcVlan(),
                    conflictDstSrc.get().getFlowId(), conflictDstSrc.get().getDestSwitch(),
        ...
        if (conflictSrcDst.isPresent()) {
            String errorMessage = format("Requested flow '%s' source conflicts with flow '%s' source. "
                    ...
                    requestedFlow.getFlowId(), requestedFlow.getDestSwitch(),
                    requestedFlow.getDestPort(), requestedFlow.getDestVlan(),
                    conflictSrcDst.get().getFlowId(), conflictSrcDst.get().getSrcSwitch(),
        ...
        if (conflictDstDst.isPresent()) {
            String errorMessage = format("Requested flow '%s' source conflicts with flow '%s' source. "
                    ...
                    requestedFlow.getFlowId(), requestedFlow.getDestSwitch(),
                    requestedFlow.getDestPort(), requestedFlow.getDestVlan(),
                    conflictDstDst.get().getFlowId(), conflictDstDst.get().getDestSwitch(),
