...
/**
 * ResponseWrapper class is a wrapper of {@link HttpServletResponseWrapper}
 * class. This will update the response output stream to custom output stream,
 * whose responsibility is to maintain a copy of output stream in parallel to
 * read the data any time without affecting the actual response data.
 * 
 * @author Gaurav Chugh
 *
 */
public class ResponseWrapper extends HttpServletResponseWrapper {
	...
	public PrintWriter getWriter() throws IOException {
		...
		if (writer == null) {
			streamWrapper = new ServletOutputStreamWrapper(getResponse()
					.getOutputStream());
			writer = new PrintWriter(new OutputStreamWriter(streamWrapper,
