...
//		_log.info("[doFilterInternal] - ");
		...
		for (String api : apis) {
			if (apiName.equalsIgnoreCase(api)
					|| fullRequestPath.toLowerCase().contains(api)) {
				isMatch = true;
			}
		}
		...
		if (isMatch)
			logRequest(request);
		ResponseWrapper responseWrapper = new ResponseWrapper(requestId,
				response);
		try {
		...
		if (60000 - elapsedTime < 0) {
			_log.info("[DelayedRequestDetail] - Time Taken: '{}', URL: '{}'",
					elapsedTime, request.getRequestURL());
		}
	}
	...
	private void logRequest(final HttpServletRequest request) {
		...
		msg.append(REQUEST_PREFIX).append("\n\tid: '")
				.append(request.getAttribute("Id")).append("', ")
	...
	private void logResponse(final ResponseWrapper response) throws JsonParseException, JsonMappingException, IOException {
		...
		try {
			...
			Object json = mapper.readValue(new String(response.getData(), response
					.getCharacterEncoding()), Object.class);
