	...
	showStatsData:function(response,metricVal,graphCode,domId,startDate,endDate,timezone) {
		...
		if(!jsonResponse) { 
		   ...
		   if(!getValue) {
		    	metric1 = "F";
		    	metric2 = "R";		    	
		    } else if(getValue && Object.keys(getValue).length) {
		    	for(i in getValue) {
		    ...
		    }else if(getVal && Object.keys(getVal).length) {
		    	for(i in getVal) {
					if(getValue[i] < 0 || getValue[i] == null ) {
						getValue[i] = 0;
					}    
		...
		if(typeof(endDate)!=='undefined' && endDate!=null){ 
			...
			var arr = [new Date(lastTime),null,null];
			graphData.push(arr);
			graphData[0]= graphData[0].splice(0,labels.length);
			graphData[graphData.length-1]= graphData[graphData.length-1].splice(0,labels.length);
		}
...
$(function() {
	...
	$("#datetimepicker7ISL,#datetimepicker8ISL,#downsamplingISL,#menulistISL,#autoreloadISL,#timezone").on("change",function(event) {
		if($("#selectedGraph").val() =='source') {
			getISLGraphData(common.toggleSwitchID(srcSwitch),srcPort);	
		}else if($("#selectedGraph").val() =='target'){
...
function callIslSwitchIntervalData(switchId,portId){ 
	...
	var switch_id =common.toggleSwitchID(switchId);//switchId.replace(/:/g, "");
	var url = "/stats/switchid/"+switch_id+"/port/"+portId+"/"+convertedStartDate+"/"+convertedEndDate+"/"+downsampling+"/"+selMetric;
