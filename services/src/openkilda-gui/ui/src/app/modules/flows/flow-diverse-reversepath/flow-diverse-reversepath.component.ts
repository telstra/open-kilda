...
@Component({
  selector: 'app-flow-diverse-reversepath',
  templateUrl: './flow-diverse-reversepath.component.html',
  styleUrls: ['./flow-diverse-reversepath.component.css']
})
export class FlowDiverseReversepathComponent implements OnInit, AfterViewInit, OnDestroy{
  ...
  ngOnInit() {
    ...
    Object.keys(this.data).map(function(i,v){
        var j = 0;
        for(let d of self.data[i].reverse_path){
        if(j < self.data[i].reverse_path.length){  j++;
            ...
            if(typeof(self.data[i].reverse_path[j]) !='undefined'){
              var nextSwitchInPort = "swr_"+self.data[i].reverse_path[j].switch_id+"_"+self.data[i].reverse_path[j].input_port;
    ...
    if(commonSwitches && commonSwitches.length){
     ...
     var svgElement = d3.select('#svgReverse');
       // creating positions for nodes
       var element =$('#ReversePathGraphwrapper');
       ...
       for(var i = 0; i < this.nodes.length; i++){
        ...
        if(xValue < maxLimit){
          ...
          if(isIslPort){
            reverseXvalue = reverseXvalue - 100;
            this.positions[this.nodes[i].switch_id][0] = reverseXvalue;
          }
           reverseXvalue = reverseXvalue - 100;
        }else{
    ...
    this.initSimulation(this.nodes,this.links,svgElement,"ReversePathGraphwrapper",'reverse');
  ...
  showCommonSwitch(){
    ...
    for(var i = 0; i < commmonSwitch.length; i++){
      ...
      var element = document.getElementById("reverse_circle_" + switch_id);
  ...
  showFlowPth(flowid){
    ...
    if(typeof(this.flowPathFlag[flowid]) !='undefined' && this.flowPathFlag[flowid]){
      d3.selectAll(".reverse_link_"+flowid)
      .transition()
      ...
      var links = this.svgElement.selectAll(".reverse_link_"+flowid);
  ...
  initSimulation(nodes,links,svgElement,graphWrapper,type){
    ...
    if (this.links.length > 0) {
      ...
      this.forceSimulation.on("end",()=>{
        this.zoomFit();
        self.loadReverseGraph = true;
        self.reversePathLoader= false;
    })
  }
    ...
    let graphLinksData = this.graphLinkGroup.selectAll("path.link").data(this.links);
     let graphNewLink = graphLinksData
      ...
      .attr("class", function(d, index) {
         return "link physical "+"reverse_link_"+d.flow;
      })
      .attr("id", (d, index) => {
      ...
      .on('mouseover',function(d,index){
        ...
        element.setAttribute('class',classes);
         ...
         $('#forward_flow_value').css('display','none');
          $("#diversepath-hover-txt").css("display", "block");
          $('#reverse_flow_value').html(d.flow);
          $('#reverse_flow_value').css('display','block');
  ...
  getPath(flowid){
    var self = this;
    var pathData = [];
    Object.keys(self.data).map(function(i,v){
      if( i == flowid && typeof(self.data[i].reverse_path) !='undefined'){
          pathData = self.data[i].reverse_path;
      }
    })
    return pathData;
