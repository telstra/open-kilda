...
public class EgressFlowSegmentRemoveCommandJsonTest extends EgressFlowSegmentBlankCommandJsonTest {

    @Override
    protected void verify(EgressFlowSegmentBlankRequest request,
                          SpeakerCommand<? extends SpeakerCommandReport> rawCommand) {
        Assert.assertTrue(rawCommand instanceof EgressFlowSegmentRemoveCommand);
        verifyPayload(request, (EgressFlowSegmentRemoveCommand) rawCommand);
    }

    @Override
    protected EgressFlowSegmentBlankRequest makeRequest(BlankResolver blank) {
