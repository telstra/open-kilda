...
import org.openkilda.northbound.DefaultNBParameters;
 ...
 */
@Service
public class FlowServiceImpl extends DefaultNBParameters implements FlowService {
	/**
	 * The logger.
	 */
	...
	public FlowStats getFlowStats(final String id, final String correlationId, final String statsType) {
		...
		if (id.equalsIgnoreCase("test-flow")) {
			...
			ForwardEgressFlow forwardEgressFlow = new ForwardEgressFlow(id, Utils.FORWARD_COOKIE_VALUE, "2", "2", "2",
					"2");
		...
		try {
			...
			result = client.target(northboundEndpoint).path("/api/v1/flows/" + id).request(MediaType.APPLICATION_JSON)
					.header(HttpHeaders.AUTHORIZATION, authHeaderValue).get(String.class);
			logger.debug("getFlow stats: {}={}" + "result : " + result);
		} catch (Exception e) {
			FlowStats response = new FlowStats();
			response.setErrorType(ErrorType.NOT_FOUND);
			response.setErrorMessage(
					"Object was not found : flow id : " + id + " ,with northboundEndpoint ip : " + northboundEndpoint);
		...
		try {
			...
			String receiveSrcPacketSwitchResult = getOpenTSDBData(correlationId, Utils.PEN_SWITCH_RX_PACKETS,
					sourceSwitchId);
			...
			String transmitSrcPacketSwitchResult = getOpenTSDBData(correlationId, Utils.PEN_SWITCH_TX_PACKETS,
					sourceSwitchId);
			...
			String receiveDstPacketSwitchResult = getOpenTSDBData(correlationId, Utils.PEN_SWITCH_RX_PACKETS,
					destinationSwitchId);
			...
			String transmitDstPacketSwitchResult = getOpenTSDBData(correlationId, Utils.PEN_SWITCH_TX_PACKETS,
					destinationSwitchId);
			...
			String receivedDstByteResult = getOpenTSDBData(correlationId, Utils.PEN_SWITCH_RX_BYTES,
					destinationSwitchId);
			...
			String transmitDstByteResult = getOpenTSDBData(correlationId, Utils.PEN_SWITCH_TX_BYTES,
					destinationSwitchId);
			...
			// flowStats.setErrorMessage("northboundEndpoint :
			// "+northboundEndpoint);
	...
	public String getOpenTSDBData(String correlationId, String metricName, String switchId) {
		...
		try {
			...
			result = client.target(opentsdbEndpoint).path("/api/query").queryParam("start", "5h-ago")
					.queryParam("m", "count:" + metricName).queryParam("{switchid=" + switchId + "}").request()
					.get(String.class);
		} catch (Exception e) {
