...
public final class CorrelationContext {
    ...
    public static String getId() {
        return Optional.ofNullable(ID.get()).orElse(MISSING_CORRELATION_ID);
    }
    ...
    public static CorrelationContextClosable append(String correlationId) {
        String currentId = ID.get();

        String newCorrelationId = correlationId;
        if (currentId != null) {
            newCorrelationId = format("%s-%s", currentId, newCorrelationId);
        }

        ID.set(newCorrelationId);
