...
public class OfFlowStatsConverterTest {
    ...
    public void testToFlowStatsData() {
        ...
        FlowStatsData data = OfFlowStatsConverter.toFlowStatsData(Collections.singletonList(ofReply), switchId);

        assertEquals(switchId, data.getSwitchId());

        assertEquals(1, data.getStats().size());
        FlowStatsReply reply = data.getStats().get(0);
        assertEquals(xId, reply.getXid());

        assertEquals(1, reply.getEntries().size());
        FlowStatsEntry entry = reply.getEntries().get(0);
    ...
    public void testFlowEntry() {
        ...
        FlowEntry entry = OfFlowStatsConverter.toFlowEntry(ofEntry);
