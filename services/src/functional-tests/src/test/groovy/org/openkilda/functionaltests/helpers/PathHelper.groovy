...
@Component
class PathHelper {
    static final String unpreferableCost = "999999999"
     ...
     * sets very big cost on that ISL, so that the flow indeed becomes less preferable.
     *
    ...
    void makePathMorePreferable(List<PathNode> morePreferablePath, List<PathNode> lessPreferablePath) {
        def islsToAvoid = getInvolvedIsls(lessPreferablePath) - getInvolvedIsls(morePreferablePath)
     ...
     * Note3: won't be able to handle situation if switchId is not present in toppology.yaml at all (will create
    ...
    static List<PathNode> convert(FlowPathPayload pathPayload) {
        ...
        if (path.empty) {
            throw new IllegalArgumentException("Path should be non-empty")
        }
        path.eachWithIndex { pathEntry, i ->
        ...
        def seqId = 0
        pathNodes = pathNodes.dropRight(1).tail()
        pathNodes.each { it.seqId = seqId++ }
        return pathNodes
    }
}
