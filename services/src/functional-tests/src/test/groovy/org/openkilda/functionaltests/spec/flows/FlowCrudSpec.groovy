...
(#flow.source.datapath - #flow.destination.datapath)")
    ...
    def "Able to create multiple flows on certain combinations of switch-port-vlans"() {
        ...
        data << [
                ...
                [
                        description  : "no vlan and vlan on the same port on dst switch",
                        getNotConflictingFlows: {
                            def (Switch srcSwitch, Switch dstSwitch) = getTopology().activeSwitches
                            def flow1 = getFlowHelper().randomFlow(srcSwitch, dstSwitch)
                            def flow2 = getFlowHelper().randomFlow(srcSwitch, dstSwitch).tap {
                ...
                [
                        description  : "no vlan and vlan on the same port on src switch",
                        getNotConflictingFlows: {
                            def (Switch srcSwitch, Switch dstSwitch) = getTopology().activeSwitches
                            def flow1 = getFlowHelper().randomFlow(srcSwitch, dstSwitch)
                            def flow2 = getFlowHelper().randomFlow(srcSwitch, dstSwitch).tap {
                ...
                [
                        description  : "vlan and no vlan on the same port on dst switch",
                        getNotConflictingFlows: {
                            def (Switch srcSwitch, Switch dstSwitch) = getTopology().activeSwitches
                            def flow1 = getFlowHelper().randomFlow(srcSwitch, dstSwitch).tap {
                                it.destination.vlanId = 0
                ...
                [
                        description  : "vlan and no vlan on the same port on src switch",
                        getNotConflictingFlows: {
                            def (Switch srcSwitch, Switch dstSwitch) = getTopology().activeSwitches
                            def flow1 = getFlowHelper().randomFlow(srcSwitch, dstSwitch).tap {
                                it.source.vlanId = 0
                ...
                [
                        description  : "no vlan and vlan on the same ports on src and dst switches",
                        getNotConflictingFlows: {
                            def (Switch srcSwitch, Switch dstSwitch) = getTopology().activeSwitches
                            def flow1 = getFlowHelper().randomFlow(srcSwitch, dstSwitch)
                            def flow2 = getFlowHelper().randomFlow(srcSwitch, dstSwitch).tap {
                ...
                [
                        description  : "vlan and no vlan on the same ports on src and dst switches",
                        getNotConflictingFlows: {
                            def (Switch srcSwitch, Switch dstSwitch) = getTopology().activeSwitches
                            def flow1 = getFlowHelper().randomFlow(srcSwitch, dstSwitch).tap {
                                it.source.vlanId = 0
    ...
    def "Systems allows to pass traffic via full and vlan port when they are on the same port"() {
        ...
        // FLOW_FULL_PORT_PRIORITY sets on an ingress rule only
        rules[srcSwitch.dpId].find { it.cookie == flowFullPortInfo.right.cookie }.priority == FLOW_PRIORITY
        rules[dstSwitch.dpId].find { it.cookie == flowFullPortInfo.left.cookie }.priority == FLOW_PRIORITY

        rules[srcSwitch.dpId].find { it.cookie == flowFullPortInfo.left.cookie }.priority == FLOW_FULL_PORT_PRIORITY
        rules[dstSwitch.dpId].find { it.cookie == flowFullPortInfo.right.cookie }.priority == FLOW_FULL_PORT_PRIORITY
