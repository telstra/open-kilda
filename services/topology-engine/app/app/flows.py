...
def build_intermediate_flows(expandedRelationships, transit_vlan, i, flow_id, outputAction):
...
def assign_meter_id():
    return random.randrange(1, 4000, 1)

def api_v1_topology_get_one_switch_flows(src_switch, src_port, src_vlan, dst_switch, dst_port, dst_vlan, bandwidth, flow_id, outputAction):
    return [[build_one_switch_flow(src_switch, src_port, src_vlan, dst_port, dst_vlan, bandwidth, flow_id, outputAction)],
            [build_one_switch_flow(dst_switch, dst_port, dst_vlan, src_port, src_vlan, bandwidth, flow_id, outputAction)]]

def api_v1_topology_get_path(src_switch, src_port, src_vlan, dst_switch, dst_port, dst_vlan,
                             bandwidth, transit_vlan, flow_id, outputAction):
    ...
    if relationships:
        ...
        while i < intermediateFlowCount:
            flows.append(build_intermediate_flows(expandedRelationships, transit_vlan, i, flow_id, outputAction))
            i += 1
    ...
    if request.method == 'POST':
        ...
        if content['src_switch'] == content['dst_switch']:
            allflows = api_v1_topology_get_one_switch_flows(
                ...
                content['bandwidth'], content['flow_id'], outputAction)
        else:
            transitVlanForward = assign_transit_vlan()
            forwardFlows = api_v1_topology_get_path(
                content['src_switch'], content['src_port'], content['src_vlan'], content['dst_switch'],
                content['dst_port'], content['dst_vlan'], transitVlanForward, flowID, outputAction)

            transitVlanReturn = assign_transit_vlan()
            reverseFlows = api_v1_topology_get_path(
                content['dst_switch'], content['dst_port'], content['dst_vlan'], content['src_switch'],
                content['src_port'], content['src_vlan'], transitVlanReturn, flowID, outputAction)
