...
public class OrientDbTinkerpopCheapestPathBenchmark extends BaseOrientDbBenchmark {
    ...
    public void crossIslandCheapestPathBenchmark(OrientDbPersistenceResources orientDbPersistenceResources,
        ...
        transactionManager.doInTransaction(() -> {
            ...
            GraphTraversal<?, ?> rawTraversal = framedGraph.traverse(input -> input.V()
                    .hasLabel(SwitchFrame.FRAME_LABEL).has(SwitchFrame.SWITCH_ID_PROPERTY,
                            startRegion.getSwitches().get(0).getSwitchId().toString())
                    .repeat(outE(IslFrame.FRAME_LABEL).has(IslFrame.STATUS_PROPERTY, "active").inV().simplePath())
                    .until(hasLabel(SwitchFrame.FRAME_LABEL).has(SwitchFrame.SWITCH_ID_PROPERTY,
                            endRegion.getSwitches().get(0).getSwitchId().toString())
                            .or().loops().is(topologyResources.maxHops))
                    .path()
                    .filter(tail(Scope.local, 1).has(SwitchFrame.SWITCH_ID_PROPERTY,
                            endRegion.getSwitches().get(0).getSwitchId().toString()))
                    .as("p")
                    .map(unfold().coalesce(__.values("cost"), constant(0)).sum()).as("c")
                    .filter(__.is(P.lt(100)))
                    .order().by(Order.decr)
                    .limit(1)
                    .select("p", "c")
            ).getRawTraversal();
    ...
    private void cheapestPath(TransactionManager transactionManager, SwitchId startSwitch, SwitchId endSwitch,
        ...
        FramedGraph framedGraph = ((FermaGraphFactory) transactionManager).getFramedGraph();
        Graph subGraph = (Graph) framedGraph.getRawTraversal().V()
                .hasLabel(SwitchFrame.FRAME_LABEL).has(SwitchFrame.SWITCH_ID_PROPERTY, startSwitch.toString())
                .repeat(outE(IslFrame.FRAME_LABEL).has(IslFrame.STATUS_PROPERTY, "active").dedup()
                        .subgraph("subGraph")
                        .inV().hasLabel(SwitchFrame.FRAME_LABEL).has(SwitchFrame.STATUS_PROPERTY, "active").dedup())
                .times(maxHops)
                .cap("subGraph").next();

        GraphTraversal<?, ?> rawTraversal = subGraph.traversal().withSack(0)
