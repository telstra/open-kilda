...
public class Neo4jTinkerpopCheapestPathBenchmark extends BaseNeo4jBenchmark {
    @Benchmark
        ...
        transactionManager.doInTransaction(() -> {
            ...
            GraphTraversal<?, ?> rawTraversal = framedGraph.traverse(input -> input.V()
                    ...
                    .until(hasLabel(SwitchFrame.FRAME_LABEL).has(SwitchFrame.SWITCH_ID_PROPERTY, endSwitch.toString())
                            .or().loops().is(maxHops))
                    .path()
                    .filter(tail(Scope.local, 1).has(SwitchFrame.SWITCH_ID_PROPERTY, endSwitch.toString()))
                    .as("p")
                    .map(unfold().coalesce(__.values("cost"), constant(0)).sum()).as("c")
                    .filter(__.is(P.lt(100)))
                    .order().by(Order.decr)
